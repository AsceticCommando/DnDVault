...
![The rampant magical experimentation in Netheril often caused strange environmental effects](file://book/NF/009-02-001.magical-experimentation.webp#center)
...
In an eerie chanting zone, which is typically no more than 300 feet in diameter, discordant chanting insinuates itself into the mind. Creatures have Disadvantage on Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) checks and on Constitution saving throws to maintain [Concentration](3-Mechanics/CLI/rules/conditions.md#Concentration).
...
In a low gravity zone, creatures quadruple the distance they can leap with a [High Jump](3-Mechanics/CLI/rules/variant-rules/high-jump-xphb.md) or a [Long Jump](3-Mechanics/CLI/rules/variant-rules/long-jump-xphb.md), and the damage creatures take from falling is halved (round down).
...
An area of unstable gravity is normally filled with debris from smashed objects that aren't affixed to a surface. A creature near an area of unstable gravity can identify its presence by noticing these smashed items, taking a [Study](3-Mechanics/CLI/rules/actions.md#Study) action, and succeeding on a DC 15 Intelligence ([Investigation](3-Mechanics/CLI/rules/skills.md#Investigation)) check.

The effect is the same as the [Reverse Gravity](3-Mechanics/CLI/spells/reverse-gravity-xphb.md) spell (save DC 16), except that it is permanent and typically covers an area that is 60 feet in diameter and 60 feet high. Every `dice:1d4|noform|noparens|avg` (`1d4`) rounds, the effect is suppressed for `dice:1d4|noform|noparens|avg` (`1d4`) rounds and normal gravity returns, causing creatures and objects to fall.
