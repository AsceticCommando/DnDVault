---
obsidianUIMode: preview
cssclasses:
- json5e-note
tags:
- ttrpg-cli/compendium/src/5e/frhof
aliases:
- "Chapter 4: Aurora's Whole Realms Catalog"
---
# Chapter 4: Aurora's Whole Realms Catalog
*Source: Forgotten Realms: Heroes of Faerûn, p. 128* 

![](file://book/FRHoF/126-frhof-ac-aurora.webp#center)

> [!readaloud] 
> 
> Welcome to Aurora's Whole Realms Catalog!
> 
> My great-grandmother founded this business over a century ago, providing adventuring gear and tools of the trade to discerning clients across Faerûn. It is my privilege to carry on in her famous footsteps.
> 
> What you see here is just a sample of the hundreds of items on sale at any of our emporium shops, located in towns and cities in every realm. If you don't see what you want at one of our shops, our special orders are swift and reliable and come with a satisfaction guarantee. We even deliver!
^2ca

## Equipment

The following equipment can be purchased through *Aurora's Whole Realms Catalog*.

### Adventuring Gear

This section describes adventuring gear used throughout the Realms.

![Adventuring Gear](3-Mechanics/CLI/tables/adventuring-gear-frhof.md)

## Mechanical Wonders

Calishites rely on the technological marvels known as [Mechanical Wonders](3-Mechanics/CLI/items/mechanical-wonder-fraif.md).

![Mechanical Wonders](3-Mechanics/CLI/tables/mechanical-wonders-frhof.md)

## Mounts and Other Animals

People in Faerûn use animals for transportation and other tasks.

![Mounts and Other Animals](3-Mechanics/CLI/tables/mounts-and-other-animals-frhof.md)

## Tools

The Musical Instruments described here are used by musicians across Faerûn.

![Tools](3-Mechanics/CLI/tables/tools-frhof.md)

## Travel

Travelers in the Realms might use interesting modes of transport.

![Travel](3-Mechanics/CLI/tables/travel-frhof.md)

### Black Coach Service (1 GP/Trip)

*Travel in the city of Baldur's Gate should be comfortable, elegant, and discreet. Narwin Cottle's magical black coaches are all three. Tell your driver Aurora sent you*.

Each of Cottle's black coaches is a Carriage (see the "Player's Handbook") keyed to a unique sigil sequence. While inside a black coach, a creature can take a [Magic](3-Mechanics/CLI/rules/actions.md#Magic) action to turn a combination dial and input a sigil sequence belonging to another black coach. All creatures inside the coach then teleport to that destination coach. If the destination coach is occupied, the teleport fails and the action is wasted.

Alternatively, a creature inside the coach can use the combination dial to input a random sequence, teleporting all creatures inside to a random coach in Cottle's fleet.

![](file://book/FRHoF/143-04-002.black-coach-service.webp#center)

### Sled Services (4 GP/Day)

*We've contracted with expert sled drivers to provide travel services around Icewind Dale, the High Ice, the Great Glacier, and other realms of the North. Each guide brings their own sled and team of six sled dogs or axe beaks. Authentic local food is provided for you and your companions. Contracts are per journey, paid half in advance and half on arrival*.

Sled service includes a [Scout](3-Mechanics/CLI/bestiary/humanoid/scout-xmm.md) and six [Axe Beaks](3-Mechanics/CLI/bestiary/monstrosity/axe-beak-xmm.md) or sled dogs ([Wolves](3-Mechanics/CLI/bestiary/beast/wolf-xmm.md)). See the "Monster Manual" for these stat blocks.

![](file://book/FRHoF/144-04-017.sled-services-and-sled-dogs.webp#center)

## Vehicles

Often found in the Dalelands, the following vehicle is used by folk looking to protect their cargo.

![Vehicles](3-Mechanics/CLI/tables/vehicles-frhof.md)