...
![](file://book/RMR/000-c1-01.webp#center)
...
> 
> ![](file://book/RMR/001-c1-02-rick.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/002-ram-head-stern.webp#center)
> 
...
> DM: You bet.
> 
> Amy (rolling a `dice:d20|noform|noparens|avg` (`d20`)): I'm pretty sneaky—that's a 17.
> 
...
> [!note] 
> 
> ![](file://book/RMR/003-ram-head-yas.webp#center)
> 
> Yeah and no.
> 
...
> [!note] 
> 
> ![](file://book/RMR/004-c1-03.webp#center)
> 
...
![](file://book/RMR/005-c1-04.webp#center)
...
> [!note] 
> 
> ![](file://book/RMR/006-ram-head-daze.webp#center)
> 
...
> 
> ![](file://book/RMR/007-ram-head-smile2.webp#center)
> 
...
> 
> ![](file://book/RMR/008-ram-head-stern.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/009-ram-head-sarcasm.webp#center)
> 
...
![](file://book/RMR/010-c1-05.webp#center)

The game uses polyhedral dice. In these rules, the different dice are referred to by the letter d followed by the number of sides: `dice:d4|noform|noparens|avg` (`d4`), `dice:d6|noform|noparens|avg` (`d6`), `dice:d8|noform|noparens|avg` (`d8`), `dice:d10|noform|noparens|avg` (`d10`) (the 0 on the die represents a 10), `dice:d12|noform|noparens|avg` (`d12`), and `dice:d20|noform|noparens|avg` (`d20`). For instance, a `dice:d6|noform|noparens|avg` (`d6`) is a six-sided die.

Percentile dice, or `dice:d100|noform|noparens|avg` (`d100`), work a little differently. You generate a number between 1 and 100 by rolling the ten-sided die twice. The first roll gives the tens digit, and the second gives the ones digit. If you roll a 7 and a 1, for example, the number rolled is 71. Two 0s represent 100.

When you need to roll dice, the rules tell you how many dice to roll of a certain type, as well as what modifiers to apply. For example, "`dice:3d8+5|noform|noparens|avg` (`3d8 + 5`)" means you roll three eight-sided dice, add them together, and add 5 to the total.

> [!note] 
> 
> ![](file://book/RMR/011-ram_d20.webp#center)
> 
...
![](file://book/RMR/012-c1-06.webp#center)
...
> [!note] 
> 
> ![](file://book/RMR/013-ram-head-rick-lol.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/014-c1-07.webp#center)
> 
...
> 
> ![](file://book/RMR/015-ram-head-rick-mad.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/016-ram-head-smile2.webp#center)
> 
> DAMN, MORTY!
> 
...
> [!note] 
> 
> ![](file://book/RMR/017-ram-head-sarcasm.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/018-ram-rick-talking.webp#center)
> 
...
Does an adventurer's sword swing hurt a dragon or bounce off its iron-hard scales? Does the ogre believe an outrageous bluff? Can a character swim across a raging river? Does a character avoid the main blast of a fireball or take full damage from the blaze? When the outcome of an action is uncertain, the game relies on the roll of a `dice:d20|noform|noparens|avg` (`d20`) to determine success or failure. Ability checks, attack rolls, and saving throws are the three main kinds of `dice:d20|noform|noparens|avg` (`d20`) rolls. For each one, you roll a `dice:d20|noform|noparens|avg` (`d20`), add any bonuses or penalties, and compare the total to a target number. If the total equals or exceeds it, you succeed. Here are those steps in more detail:

#### 1. Roll the die

Roll a `dice:d20|noform|noparens|avg` (`d20`), then add the modifier of the ability score used for the roll. A Dexterity saving throw, for instance, uses your Dexterity modifier.
...
![](file://book/RMR/019-c1-08.webp#center)
...
Sometimes a special ability or spell tells you that you have advantage or disadvantage on a `dice:d20|noform|noparens|avg` (`d20`) roll. When that happens, you roll a second `dice:d20|noform|noparens|avg` (`d20`) when you make the roll. Use the higher of the two rolls if you have advantage, and use the lower roll if you have disadvantage. For example, if you have disadvantage and roll a 17 and a 5, you use the 5. If you instead have advantage and roll those numbers, you use the 17.

If multiple situations affect a roll and each one grants advantage or imposes disadvantage on it, you don't roll more than one additional `dice:d20|noform|noparens|avg` (`d20`). If two favorable situations grant advantage, for example, you still roll only one additional `dice:d20|noform|noparens|avg` (`d20`).

If circumstances cause a roll to have both advantage and disadvantage, you are considered to have neither of them, and you roll one `dice:d20|noform|noparens|avg` (`d20`). This is true even if multiple circumstances impose disadvantage and only one grants advantage or vice versa. In such a situation, you have neither advantage nor disadvantage.

When you have advantage or disadvantage and something in the game lets you reroll the `dice:d20|noform|noparens|avg` (`d20`), you can reroll only one of the dice. You choose which one.

> [!note] 
> 
> ![](file://book/RMR/020-c1-09.webp#center)
> 
...
To make an ability check, roll a `dice:d20|noform|noparens|avg` (`d20`) and add the appropriate ability modifier. You use your Strength modifier for a Strength check, for example.
...
> 
> ![](file://book/RMR/021-ram-head-smile2.webp#center)
> 
...
> 
> ![](file://book/RMR/022-ram-head-ehlol.webp#center)
> 
...
> 
> ![](file://book/RMR/023-ram-head-smile1.webp#center)
> 
...
![](file://book/RMR/025-c1-11.webp#center)
...
> [!note] 
> 
> ![](file://book/RMR/026-ram-head-smile1.webp#center)
> 
...
> 
> ![](file://book/RMR/027-ram-mortyface-panic.webp#center)
> 
...
> 
> ![](file://book/RMR/028-ram-head-stern.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/029-c1-13.webp#center)
> 
> ACROBATICS
> 
...
> ***EVEN CLERICS!***
> 
> ![](file://book/RMR/030-ram-head-drunk.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/031-ram-head-rick-lol.webp#center)
> 
> STEALTH
> 
...
> [!note] 
> 
> ![](file://book/RMR/032-c1-14.webp#center)
> 
> S-s-sure you can memorize all this if you want: passive blah-blah and how much of a character's ass is sticking out or whatever, but if you wanna do this Rick-style, here's how it goes: roll a `dice:d20|noform|noparens|avg` (`d20`) behind your DM screen and then—it *doesn't matter what the roll was*, just look at the players, squint your eyes a bit, and say:
> 
...
> [!note] 
> 
> ![](file://book/RMR/033-ram-head-startled.webp#center)
> 
...
> 
> ![](file://book/RMR/034-c1-15.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/035-ram-head-rick-madconfused.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/036-c1-16.webp#center)
> 
> ANIMAL HANDLING
> 
...
> 
> ![](file://book/RMR/037-ram-head-rick-lol.webp#center)
> 
...
> [!note] 
> 
> ![](file://book/RMR/038-c1-17.webp#center)
> 
> Medicine
> 
...
> [!note] 
> 
> ![](file://book/RMR/039-ram-head-daze.webp#center)
> 
> PERCEPTION
> 
...
> [!note] 
> 
> ![](file://book/RMR/040-c1-18.webp#center)
> 
> SURVIVAL
> 
...
To make a saving throw, roll a `dice:d20|noform|noparens|avg` (`d20`) and add the appropriate ability modifier. For example, you use your Dexterity modifier for a Dexterity saving throw.
...
![](file://book/RMR/041-c1-19.webp#center)