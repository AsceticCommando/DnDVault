---
obsidianUIMode: preview
cssclasses:
- json5e-item
tags:
- ttrpg-cli/compendium/src/5e/imr
- ttrpg-cli/item/rarity/none
aliases:
- "Thessaltoxin"
---
# Thessaltoxin
*Poison (ingested or injury)*  


- **Properties**: [Poison](3-Mechanics/CLI/rules/item-properties.md#Poison)

This poison was first created using blood harvested from the artificer Thessalar. A creature subjected to the poison must succeed on a DC 15 Constitution saving throw or take on a new form as if affected by the [polymorph](3-Mechanics/CLI/spells/polymorph-xphb.md) spell. The creature's new form is a random beast or a creature it has seen within the last 24 hours (as chosen by the DM). This transformation lasts until the end of the target's next long rest. This effect is not subject to [dispel magic](3-Mechanics/CLI/spells/dispel-magic-xphb.md) or [remove curse](3-Mechanics/CLI/spells/remove-curse-xphb.md), but a [greater restoration](3-Mechanics/CLI/spells/greater-restoration-xphb.md) spell restores the creature to its original form.

*Source: Infernal Machine Rebuild p. 93*