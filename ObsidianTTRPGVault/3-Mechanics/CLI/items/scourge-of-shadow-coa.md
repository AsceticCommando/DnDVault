...
The Scourge of Shadow is a magic whip. You have a +3 bonus to attack and damage rolls made with this weapon, and it does `dice:3d4|noform|noparens|avg` (`3d4`) slashing damage instead of its regular damage. When you hit a good aligned creature with it, that creatures takes an extra `dice:4d4|noform|noparens|avg` (`4d4`) necrotic damage.

Whenever an unattuned creature attacks with the weapon, it deals `dice:2d4|noform|noparens|avg` (`2d4`) necrotic damage, ignoring resistances and immunities, to them, but otherwise functions normally.
...
The whip scores a critical on a roll of 18, 19, or 20. When it scores a critical, its damage dice are upgraded to `dice:3d6|noform|noparens|avg` (`3d6`) instead of `dice:3d4|noform|noparens|avg` (`3d4`) for the duration of the attack. If attacking a good aligned creature, or attacking with the weapon unattuned, the necrotic damage dice are also increased to a `dice:d6|noform|noparens|avg` (`d6`) instead of a `dice:d4|noform|noparens|avg` (`d4`).
...
You're protected by a field of shadow that makes you resistant to radiant and necrotic damage. While in dim light or darkness you gain [blindsight](3-Mechanics/CLI/rules/senses.md#Blindsight) out to 60 feet and Dexterity ([Stealth](3-Mechanics/CLI/rules/skills.md#Stealth)) checks are made with advantage.
