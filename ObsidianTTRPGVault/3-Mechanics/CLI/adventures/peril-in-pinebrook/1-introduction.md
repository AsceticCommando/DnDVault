...
![](file://adventure/PiP/001-0000.webp#center)
...
To play this game, you need this packet and some dice: at least one 20-sided die (`dice:d20|noform|noparens|avg` (`d20`)) and one 6-sided die (`dice:d6|noform|noparens|avg` (`d6`)). It speeds up game play if each player has their own dice, but players can share! If you don't have these dice, you can find digital dice rollers online.
...
![](file://adventure/PiP/0000.webp#center)
...
You find the answer to these questions during the game by making a `dice:d20|noform|noparens|avg` (`d20`) roll!

### d20 Rolls

The 20-sided die (`dice:d20|noform|noparens|avg` (`d20`)) is the most-used die in D&D. It is rolled whenever a character or a creature tries to do something important.

Things a character might try to do are listed under "Attacks" and "Skills" on the character sheets. Next to attacks and skills are number expressions, such as "`dice:1d20+5|noform|noparens|text(+5)` to hit." This is the number you add to your die roll when your character uses that attack or skill.

For example, the character sheet for Shalefire Stoutheart lists "Handaxe" under "Attacks," with "`dice:1d20+6|noform|noparens|text(+6)` to hit" next to it. If Shalefire wants to attack a monster with their handaxe, the player rolls a `dice:d20|noform|noparens|avg` (`d20`). If the die roll is a 10, the player adds 6 to the die roll for a final attack score of 16.
...
When an attack roll hits, it deals damage. The damage roll appears after the attack on the character sheet. Shalefire's handaxe does "`dice:1d6+4|noform|noparens|avg` (`1d6 + 4`) damage," so the player would roll one 6-sided die (`dice:d6|noform|noparens|avg` (`d6`)) and add 4 to the die roll.
...
Advantage means that instead of rolling the `dice:d20|noform|noparens|avg` (`d20`) once for the attack roll, Shalefire's player rolls the `dice:d20|noform|noparens|avg` (`d20`) twice and uses the higher roll. They then add the normal modifier (+6 for Shalefire's handaxe) to get a final attack result.

#### Disadvantage

Disadvantage works like advantage in reverse. If something in the game makes it harder to take an action, the roll for that action is made with disadvantage. For example, as Shalefire climbs a cliff using a rope, a monster at the top of the cliff shakes the rope. Because it's now harder to climb, Shalefire makes the `dice:d20|noform|noparens|avg` (`d20`) roll to climb the rope with disadvantage.

Disadvantage means that instead of rolling the `dice:d20|noform|noparens|avg` (`d20`) once to make the [Athletics](3-Mechanics/CLI/rules/skills.md#Athletics) check to climb, Shalefire's player rolls the `dice:d20|noform|noparens|avg` (`d20`) twice and uses the lower roll. They then add the normal modifier (+6 for Shalefire's [Athletics](3-Mechanics/CLI/rules/skills.md#Athletics) skill) to get a final result.
...
When the action is somewhere between easy and impossible, trying to do it could lead to success or failure. That's when the DM calls for a `dice:d20|noform|noparens|avg` (`d20`) roll. See the "`dice:d20|noform|noparens|avg` (`d20`) Rolls" section earlier in this document.

The `dice:d20|noform|noparens|avg` (`d20`) roll, plus its modifiers, determines whether the action succeeds or fails. What does success or failure mean for an adventure? This is where you and the players get to tell a story together!

### Narrating Successes and Failures

When a `dice:d20|noform|noparens|avg` (`d20`) roll succeeds or fails, the DM and the players can turn that result into a part of the ongoing story. What does it look like when a monster is defeated? What does Shalefire say or do if the heavy boulder blocking the cave doesn't budge? These points in the game can spark great storytelling and acting moments.
...
![](file://adventure/PiP/0002.webp#center)
...
Explain how `dice:d20|noform|noparens|avg` (`d20`) rolls work with the following example:

> [!readaloud] 
> 
> The character Noorah Eldenfield has a shortbow attack that says "`dice:1d20+5|noform|noparens|text(+5)` to hit." When Noorah attacks with a shortbow, Noorah's player rolls a `dice:d20|noform|noparens|avg` (`d20`) and adds 5 to the number rolled on the die. I'll tell you if that attack hits the monster. If it does, Noorah's shortbow attack also says "Damage `dice:1d6+3|noform|noparens|avg` (`1d6 + 3`)." The player rolls a 6-sided die and adds 3 to determine the damage dealt by the attack.
^01c
...
![](file://adventure/PiP/0004.webp#center)
...
![](file://adventure/PiP/001-0049.webp#center)
...
After Captain Kole leaves, the characters should attempt to find food for the dragon. You may want to point out that all the characters have rations listed on their character sheets under "Other Equipment." Alternatively, Shalefire can make a DC 10 [Survival](3-Mechanics/CLI/rules/skills.md#Survival) check or Gallantine can make a DC 10 [Nature](3-Mechanics/CLI/rules/skills.md#Nature) check to find berries and nuts the dragon can eat. Remind players how to make `dice:d20|noform|noparens|avg` (`d20`) rolls if needed.
...
> 
> > [!example] Attack
> > **Claws.** *Melee Weapon Attack:* `dice:1d20+2|noform|noparens|text(+2)` to hit. *Hit:* `dice:1d6|noform|noparens|avg` (`1d6`) slashing damage.
^living-icicle
...
Roll a `dice:d20|noform|noparens|avg` (`d20`) and add the attack modifier (+2) to the roll. If the total equals or exceeds the Armor Class of the character being attacked, the attack hits. Roll `dice:1d6|noform|noparens|avg` (`1d6`), and tell the player to subtract that damage from their hit points. If the attack misses, nothing happens. Either way, describe the attack. Do the claws rend open a wound or rake harmlessly off armor?
...
![](file://adventure/PiP/0005.webp#center)
...
The challenge descriptions suggest potential ways characters can overcome each challenge. However, if the players come up with other ways that might work, let them make a `dice:d20|noform|noparens|avg` (`d20`) roll that somehow connects to their idea. Imagination often leads to an even more creative story!
...
Characters must succeed on a DC 10 [Athletics](3-Mechanics/CLI/rules/skills.md#Athletics) or [Acrobatics](3-Mechanics/CLI/rules/skills.md#Acrobatics) check to climb the wall. If a character fails the check, roll `dice:1d6|noform|noparens|avg` (`1d6`). Tell the player to subtract that number from the character's hit points to represent the damage the character takes from falling before they eventually reach the top.
...
![](file://adventure/PiP/001-0103.webp#center)
...
> 
> > [!example] Attack
> > **Bite.** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit. *Hit:* `dice:1d6+2|noform|noparens|avg` (`1d6 + 2`) piercing damage.
^egg-snatchers
...
![](file://adventure/PiP/0006.webp#center)
...
> [!gallery]
> ![](file://adventure/PiP/0007.webp#gallery)
> ![](file://adventure/PiP/0010.webp#gallery)