...
![An infernal warship docks at Kordichai's manor.](file://adventure/CoA/108-0.webp#center)
...
| 2 | An infestation of two dozen [fire snakes](3-Mechanics/CLI/bestiary/elemental/salamander-fire-snake-xmm.md) flees from five [barbed devils](3-Mechanics/CLI/bestiary/fiend/barbed-devil-xmm.md) who hunt them. The vermin broke into an infernal store house and devoured a stockpile of weapons. The devils offer one [Soul Coin](3-Mechanics/CLI/items/soul-coin-bgdia.md) for each head. |
| 3 | A pack of six [hell hounds](3-Mechanics/CLI/bestiary/fiend/hell-hound-xmm.md), led by a powerful [hellcat](3-Mechanics/CLI/bestiary/fiend/hellcat-bezekira-coa.md) (see appendix B), roam the wastes looking for food. |
| 4 | Devils native to Phlegethos have frequent competitions called "fireflights," wherein the devils fly low and fast across the fiery plane. Each character must make a DC 20 Dexterity saving throw. On a failed save, a spined devil collides with them and the character falls, landing with the [prone](3-Mechanics/CLI/rules/conditions.md#Prone) condition and taking `dice:4d4\|noform\|noparens\|avg\|text(10)` (`4d4`) piercing damage and `dice:4d6\|noform\|noparens\|avg\|text(14)` (`4d6`) fire damage. On a successful save, the character avoids the collision. If a collision occurs, the [spined devil](3-Mechanics/CLI/bestiary/fiend/spined-devil-xmm.md) attacks, angry that the character has cost them the race. |
| 5 | A small group of experienced [imps](3-Mechanics/CLI/bestiary/fiend/imp-xmm.md) lead travelers across Phlegethos and into Abriymoch proper. They claim to know the safest routes and offer their services for a platinum coin per person. If the characters pay this fee, they may travel to Abriymoch without any further random encounters. |
| 6 | Fire, smoke, and wind descend upon the wastes. Visibility becomes severely reduced, and creatures without fire resistance or immunity take `dice:1d6\|noform\|noparens\|avg\|text(3)` (`1d6`) fire damage at the start of each of their turns unless they find cover. The storm lasts `dice:2d4\|noform\|noparens\|avg` (`2d4`) hours and halfway through its duration a rogue explosion occurs. This acts as if a [Fireball](3-Mechanics/CLI/spells/fireball-xphb.md) spell with a save DC of 20 has detonated, centered around one of the characters. |
^random-encounters-in-phlegethos
...
> [!gallery]
> ![Phlegethos, the Fourth Hell](file://adventure/CoA/110-0-dm.webp#gallery)
> ![Player Version](file://adventure/CoA/110-0.webp#gallery)
...
![Even after the skies darken over Phlegethos, there is little respite from the oppressive heat.](file://adventure/CoA/111-0.webp#center)
...
Outside the city of Abriymoch, the heat is unmatched and overwhelming. Visitors take `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) fire damage each hour they spend in the layer. Creatures with resistance or immunity to fire are immune to this damage. Damage taken from exposure to Phlegethos hostile environment doesn't interrupt rests unless it would cause a creature to have the [unconscious](3-Mechanics/CLI/rules/conditions.md#Unconscious) condition.
...
> ***"The Heretic Priest"***
> 
> A priest of an opposing faith to one of the characters (select the most faithful among the party) is speaking with a [succubus](3-Mechanics/CLI/bestiary/fiend/succubus-xmm.md). If the party spends any amount of time eavesdropping, then run "The Heretic Priest" event from chapter 2.
...
> 
> The characters can target the bracer with melee or ranged attacks (AC 18; 50 hit points; immunity to cold, fire, poison and psychic damage; resistance to all other damage types). If [Anagwendol](3-Mechanics/CLI/bestiary/npc/anagwendol-coa.md) is [grappled](3-Mechanics/CLI/rules/conditions.md#Grappled), [incapacitated](3-Mechanics/CLI/rules/conditions.md#Incapacitated), or [restrained](3-Mechanics/CLI/rules/conditions.md#Restrained), attacks made against the bracer are made with advantage.
^the-infernal-bracer
...
If the characters ever stop the creature from shoveling coal it whimpers and then vanishes. A moment later there is a shriek and the creature reappears, scrambling out of the furnace. Any other creature that enters the furnace, or starts their turn within the furnace, takes `dice:12d12|noform|noparens|avg|text(78)` (`12d12`) fire damage.
...
But first they must overcome her. At the end of each hour meandering through the dark, twisting tunnels, roll a `dice:d4|noform|noparens|avg` (`d4`) (adding 1 for each previous roll). Once the result is 4 or higher, [Anagwendol](3-Mechanics/CLI/bestiary/npc/anagwendol-coa.md) strikes. She emerges from hiding, and if not detected, she grapples a character and carries them straight upwards, through a vertical tunnel, depositing them in a chamber 60 feet above their companions. She then attacks them. If she slays her victim, she repeats the ambush `dice:1d4|noform|noparens|avg` (`1d4`) hours later against another character.

> [!gallery]
> ![The Elemental Preserve](file://adventure/CoA/115-0-dm.webp#gallery)
> ![Player Version](file://adventure/CoA/115-0.webp#gallery)

##### The Hunt

If her captives are helped by allies—or if she fails to surprise the characters—she fights for 3 rounds, then attempts to disappear down one of the hallways. Whenever the characters lose sight of her, they must track her, requiring a successful DC 15 Wisdom ([Survival](3-Mechanics/CLI/rules/skills.md#Survival)) check.
...
Anyone approaching the manor is intercepted by two [horned devils](3-Mechanics/CLI/bestiary/fiend/horned-devil-xmm.md), each with a pack of four [hell hounds](3-Mechanics/CLI/bestiary/fiend/hell-hound-xmm.md). They demand that intruders turn back, but a donation of 5 [Soul Coins](3-Mechanics/CLI/items/soul-coin-bgdia.md) or 4,000 gp convinces them to set up an audience with Kordichai. Proceed to "An Important Audience" in the "Kordichai's Hunt" section below.
...
An audience with Kordichai can go four ways depending on how convincing the characters are. Have them make a Charisma ([Deception](3-Mechanics/CLI/rules/skills.md#Deception) or [Persuasion](3-Mechanics/CLI/rules/skills.md#Persuasion)) check and based on the result, one of the following occurs:
...
![A fire kraken holds its own against a fleet of infernal warships.](file://adventure/CoA/117-0.webp#center)
...
Ranged Weapon Attack: `dice:1d20+6|noform|noparens|text(+6)` to hit, range 120/480 ft., one target. Hit: `dice:3d10|noform|noparens|avg|text(16)` (`3d10`) cold damage. A creature with fire resistance or immunity is considered vulnerable to the damage of this bolt.
...
As beaters, the characters join a group that includes a [bone devil](3-Mechanics/CLI/bestiary/fiend/bone-devil-xmm.md), a pair of [azers](3-Mechanics/CLI/bestiary/elemental/azer-sentinel-xmm.md), a [salamander](3-Mechanics/CLI/bestiary/elemental/salamander-xmm.md) and a tiefling [scout](3-Mechanics/CLI/bestiary/humanoid/scout-xmm.md). These beaters follow the leadership of whoever the characters appoint as captain. Their job is to drive the prey toward Kordichai's barge. To do this, they need to spread out in a formation that has them 30 feet away from the nearest beater. Every hour, the designated captain must make a DC 15 Wisdom ([Survival](3-Mechanics/CLI/rules/skills.md#Survival)) check. Success results in a roll on the Hunt Random Encounters table. Each failure angers Kordichai. Three failures in a row and Kordichai calls the hunt off and sends a half-dozen [horned devils](3-Mechanics/CLI/bestiary/fiend/horned-devil-xmm.md) to dispose of the beaters. The characters are now treated as 'prey' and should use the appropriate encounters during the remainder of their exploration of the preserve.
...
If Kordichai designates the characters as prey, then they're told of a location in the Preserve where they can meet the other prey. If the characters are looking for the prey on their own, they can use divination magic or tracking—a successful DC 20 Wisdom ([Survival](3-Mechanics/CLI/rules/skills.md#Survival)) check—to locate them. The prey consists of a human [veteran](3-Mechanics/CLI/bestiary/humanoid/warrior-veteran-xmm.md), elf [mage](3-Mechanics/CLI/bestiary/humanoid/mage-xmm.md), dwarf [gladiator](3-Mechanics/CLI/bestiary/humanoid/gladiator-xmm.md) and tiefling [priest](3-Mechanics/CLI/bestiary/humanoid/priest-xmm.md). They look to the characters for leadership. They know some of the locations and denizens of the Preserve, including the hunting lodges, the [efreeti](3-Mechanics/CLI/bestiary/elemental/efreeti-xmm.md) and the [Eriflamme](3-Mechanics/CLI/bestiary/npc/eriflamme-coa.md). Each hour, the prey must make a DC 20 Wisdom ([Survival](3-Mechanics/CLI/rules/skills.md#Survival)) check to avoid the dangers of the Preserve. Failure means a roll on the Hunt Random Encounters table. A failure of 10 or less means that Kordichai's barge has found them. They can also try to find a specific location or denizen such as a hunting lodge. This requires a successful DC 15 Wisdom ([Survival](3-Mechanics/CLI/rules/skills.md#Survival)) check, but also means that they can't avoid the dangers and so must roll on the Hunt Random Encounters table, even if they succeed.
...
A single [phoenix](3-Mechanics/CLI/bestiary/elemental/phoenix-mpmm.md) (see Monsters of the Multiverse) is encountered.
...
![The mighty Eriflamme has eluded Kordichai for millennia.](file://adventure/CoA/119-0.webp#center)
...
![A Hunting Lodge.](file://adventure/CoA/121-0.webp#center)

At any time, the characters might attempt to use trickery to obtain the item (such as casting a spell like [Telekinesis](3-Mechanics/CLI/spells/telekinesis-xphb.md) to steal one of the artifacts right off their persons). But it is challenging to do so without the archdevil or witnesses noticing.
