...
![](file://adventure/HftT/000-c101.webp#center)
...
He starts with two [bags of holding](3-Mechanics/CLI/items/bag-of-holding-xdmg.md), so that they can easily carry the thessalhydra's severed heads. Then, for each character, he provides a [ring of protection](3-Mechanics/CLI/items/ring-of-protection-xdmg.md) and a [potion of healing](3-Mechanics/CLI/items/potion-of-healing-xdmg.md). Finally, he has two weapons—a [wand of magic missiles](3-Mechanics/CLI/items/wand-of-magic-missiles-xdmg.md) and this greatsword:

[Winter's Dark Bite](3-Mechanics/CLI/items/winters-dark-bite-hftt.md). This greatsword is made of an unknown black metal. In most cases, it works as a [+1 greatsword](3-Mechanics/CLI/items/1-weapon-xdmg.md). But when used against a thessalhydra, it works as a [+3 greatsword](3-Mechanics/CLI/items/3-weapon-xdmg.md). While in the Upside Down, it works as a +4 greatsword.
...
When the players start getting bored with roleplaying encounters, it's time for them to find the [troglodyte](3-Mechanics/CLI/bestiary/monstrosity/troglodyte-xmm.md) caves. If they've wandered off following false leads, just have them luckily stumble upon the right cave.
...
If the skill check fails, they still arrive at the caves, but unluckily do so at the same time as a returning hunting party of 8 [troglodytes](3-Mechanics/CLI/bestiary/monstrosity/troglodyte-xmm.md). They must defeat the hunting party before they can enter the caves.

![](file://adventure/HftT/001-c102.webp#center)
...
The cave is empty, but the hall to area 4 is trapped, which requires a successful DC 10 ability check to detect, using Intelligence ([Investigation](3-Mechanics/CLI/rules/skills.md#Investigation)), or disarm, using Dexterity. If triggered, the roof collapses. Anyone in the hall must make a DC 10 Dexterity save. Those who fail take `dice:2d6+2|noform|noparens|avg` (`2d6 + 2`) bludgeoning damage. Those who succeed take half as much damage. The hall becomes impassable.
...
This is where most of the troglodytes sleep. Ten [troglodytes](3-Mechanics/CLI/bestiary/monstrosity/troglodyte-xmm.md) are here, but they are terrified and try to run away into area 6 as soon as they see the characters.

### Area 6: Noble Chamber

This is where the five [troglodyte](3-Mechanics/CLI/bestiary/monstrosity/troglodyte-xmm.md) leaders sleep. The head trog is a fair and noble ruler (like a trog version of Sir Tristan). He'd rather talk than fight (unlike most trogs, he speaks Common). But if the group angers him, all the trogs attack and fight to the death. If the group politely asks about the thessalhydra, he'll say that it comes in from the woods, passes through the caves, and enters the Cursed Labyrinth. It never travels the opposite way.

### Area 7: False Treasure Room

This room is like area 4, but everything is broken and worthless. One vase is trapped, requiring a successful DC 10 Intelligence ([Investigation](3-Mechanics/CLI/rules/skills.md#Investigation)) check to detect and a successful DC 10 Dexterity check to disarm. If triggered, the hall to area 5 collapses. Anyone in the hall must make a DC 10 Dexterity save. Those who fail take `dice:2d6+2|noform|noparens|avg` (`2d6 + 2`) bludgeoning damage. Those who succeed take half as much damage. The hall becomes impassable.

### Area 8: Guard Post

Eight [troglodyte](3-Mechanics/CLI/bestiary/monstrosity/troglodyte-xmm.md) guards are stationed here. If the characters enter or make too much noise in an adjoining room (for example, have a fight or argument), the guards attack.

### Area 9: Owlbear

One [owlbear](3-Mechanics/CLI/bestiary/monstrosity/owlbear-xmm.md) is here and sleeping. Characters may attempt Dexterity ([Stealth](3-Mechanics/CLI/rules/skills.md#Stealth)) checks against its passive Perception (13). Failure means it wakes up and attacks.

### Area 10: Water Pool

A natural spring feeds a 10-foot-deep pool of clear, fresh water. In the pool lives a [giant frog](3-Mechanics/CLI/bestiary/beast/giant-frog-xmm.md), which leaps out and attacks the first character to touch the water.
...
| d20 Result | Result |
|------------|--------|
| 1–2 | `dice:1d10\|noform\|noparens\|avg` (`1d10`) map squares straight, then dead end |
| 3–5 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) map squares straight, then reroll |
| 6–8 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) map squares straight, then turn left |
| 9–11 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) map squares straight, then turn right |
| 12–14 | Special encounter (described below) |
| 15–16 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) map squares straight, then T intersection |
| 17–18 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) map squares straight, then a four-way intersection |
...
They find the remains of an ancient adventurer who died trying to escape the labyrinth. The body has treasure worth `dice:3d20|noform|noparens|avg` (`3d20`) gold pieces.

#### Pit Trap

The hallway has a pit trap somewhere along its length. Characters must make a successful DC 12 Intelligence ([Investigation](3-Mechanics/CLI/rules/skills.md#Investigation)) check to detect the trap, and a successful DC 12 Dexterity check to safely move past it. Those who fail the Dexterity check fall in and take `dice:1d6|noform|noparens|avg` (`1d6`) bludgeoning damage. Climbing out of the pit is easy to do.

#### Troglodytes

Four [troglodytes](3-Mechanics/CLI/bestiary/monstrosity/troglodyte-xmm.md), members of the tribe from the caverns who ran into the labyrinth to escape the thessalhydra, have been trapped here for two weeks. They're mad with hunger and immediately attack.

#### Skeletons

Eight [skeletons](3-Mechanics/CLI/bestiary/undead/skeleton-xmm.md), the undead remains of adventurers, come shambling down the hallway to attack the characters.
...
![](file://adventure/HftT/002-c103.webp#center)
...
To solve the riddle, the characters must put the right statue from the table in the empty space on shelf 3. If they put a wrong sculpture on the shelf, everyone in the room takes `dice:1d4|noform|noparens|avg` (`1d4`) lightning damage. They must try again. If they put the right statue there, the door opens.
...
![](file://adventure/HftT/003-c104.webp#center)
...
![](file://adventure/HftT/004-c105.webp#center)
...
![](file://adventure/HftT/005-c106.webp#center)
...
![](file://adventure/HftT/007-c108.webp#center)
...
![](file://adventure/HftT/008-c109.webp#center)
...
There's a 20-foot-wide pit. It takes a successful DC 12 Strength ([Athletics](3-Mechanics/CLI/rules/skills.md#Athletics)) check to jump across. Failing a jump without a safety rope tied to you means you fall away into darkness and take `dice:3d10|noform|noparens|avg` (`3d10`) bludgeoning damage. The pit is 30 feet deep.
...
A tribe of ten [troglodytes](3-Mechanics/CLI/bestiary/monstrosity/troglodyte-xmm.md) live here. When the thessalhydra moved in, the trogs began to worship it as a god. Now they protect it while it sleeps. If the characters have already killed the thessalhydra, the trogs begin to worship them as their new gods.

### Area 5

At the back of this cave is an area that appears wet, like a mud patch or a shallow puddle, and occasionally ripples or bubbles. It's actually an [ochre jelly](3-Mechanics/CLI/bestiary/ooze/ochre-jelly-xmm.md). If the characters don't bother it, it won't bother them.
...
![](file://adventure/HftT/009-c110.webp#center)
