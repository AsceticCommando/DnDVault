...
> 
> ![](file://adventure/TftYP-ToH/000-totyp-07-book.webp#center)
> 
...
> 
> Characters who become astral or ethereal in the tomb might attract a demon. Roll a `dice:d20|noform|noparens|avg` (`d20`) whenever this occurs.
> 
...
> [!gallery]
> ![Map 7.1: Tomb of Horrors](file://adventure/TftYP-ToH/001-toh01.webp#gallery)
> ![Player Version](file://adventure/TftYP-ToH/002-toh01_players.webp#gallery)
...
If the ceiling is prodded with any force, or if the doors are opened, the ceiling of the tunnel collapses. Creatures under the collapse must make a DC 15 Dexterity saving throw, taking `dice:5d10|noform|noparens|avg|text(27)` (`5d10`) bludgeoning damage on a failed save, or half as much damage on a successful one. The trap mechanism is beyond the doors and inside the ceiling, so it can't be disabled from outside.
...
![](file://adventure/TftYP-ToH/002-totyp-07-03.webp#center)
...
The needle deals 1 piercing damage and `dice:2d10|noform|noparens|avg|text(11)` (`2d10`) poison damage if it hits someone. Someone who takes this poison damage must succeed on a DC 15 Constitution saving throw or become [poisoned](3-Mechanics/CLI/rules/conditions.md#Poisoned) for 1 hour.
...
> 
> Each pit is spiked at the bottom, so someone who falls in takes falling damage plus `dice:2d10|noform|noparens|avg|text(11)` (`2d10`) piercing damage from the spikes. The spikes are also [poisoned](3-Mechanics/CLI/rules/conditions.md#Poisoned), so someone injured by them must make a DC 15 Constitution saving throw, taking `dice:4d10|noform|noparens|avg|text(22)` (`4d10`) poison damage on a failed save, or half as much damage on a successful one.
^covered-pits
...
> F. Slide left
> 
> G. Door has seven studs in a row—press all at once and door opens, but press 1 and 7 and the door falls inward. A character pressing the studs or near enough to do so must succeed on a DC 15 Dexterity saving throw or take `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) bludgeoning damage from the falling door.
^04d

#### Bolts from Above

Each round that characters are in any of the rooms inside the complex of secret doors, a number of bolts will be fired into the area from hidden devices in the walls and ceilings, and there is a 50 percent chance that one character, randomly determined, in each such area must succeed on a DC 20 Dexterity saving throw or take `dice:1d10|noform|noparens|avg|text(5)` (`1d10`) piercing damage. There is no way to prevent the bolts from being triggered.
...
![](file://adventure/TftYP-ToH/003-totyp-07-04.webp#center)
...
In four locations in the tomb, there are false doors which screen a spear trap. When one of these doors is opened, a spear will shoot out, and the door opener or someone standing nearby is subject to be struck. Determine at random, if need be, which character is the target. The trap has a range of 60 feet, has a `dice:1d20+11|noform|noparens|text(+11)` bonus to hit, and deals `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) piercing damage. If the door is closed and reopened, another spear will fire. The mechanism can be jammed by a character who uses thieves' tools and succeeds on a DC 20 Dexterity check.
...
![](file://adventure/TftYP-ToH/004-totyp-07-05.webp#center)
...
The middle vat contains a slow-acting acid which will deal `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) acid damage on the round after a creature's flesh comes into substantial contact with it (by immersing an arm, being splashed on, and so forth)—minor contact (dipping a finger) will cause only a mild itch. At the bottom of this vat is one-half of a golden key.
...
![](file://adventure/TftYP-ToH/005-totyp-07-06.webp#center)
...
The trunks are empty, but the smaller coffers hold various items. Roll a `dice:d6|noform|noparens|avg` (`d6`) each time one is opened. On a roll of 1–3, the coffer contains `dice:1d3|noform|noparens|avg` (`1d3`) [poisonous snakes](3-Mechanics/CLI/bestiary/beast/venomous-snake-xmm.md). On a roll of 4–5, `dice:8d10|noform|noparens|avg` (`8d10`) pp are found. On a roll of 6, the coffer holds `dice:2d4|noform|noparens|avg` (`2d4`) gems (worth 10 gp each).
...
To determine the contents of the small sack, roll a `dice:d10|noform|noparens|avg` (`d10`):
...
| 5–6 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) [potions of greater healing](3-Mechanics/CLI/items/potion-of-greater-healing-xdmg.md) |
| 7 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) [spell scrolls](3-Mechanics/CLI/items/spell-scroll-xdmg.md), each of a wizard spell of 5th level or lower (DM's choice) |
| 8 | `dice:4d12\|noform\|noparens\|avg` (`4d12`) gems (worth 100 gp each) |
...
Every 10 minutes after the gas is released, roll a `dice:d4|noform|noparens|avg` (`d4`). On a roll of 4, a stone juggernaut (rather like a steam roller) comes out of the 20-foot-square room to the north and rolls `1d6 × 10` feet south, then west. Everything it rolls over is squashed to a pulp. There is no appeal.
...
![](file://adventure/TftYP-ToH/006-totyp-07-08.webp#center)
...
There are numerous skeletons here, rotting equipment, and `dice:30d10|noform|noparens|avg` (`30d10`) each of sp, ep, gp, and pp hurled about. Searching through the remains will yield `dice:2d10|noform|noparens|avg` (`2d10`) gems (worth 10 gp each), a [potion of diminution](3-Mechanics/CLI/items/potion-of-diminution-xdmg.md), and a [+1 flail](3-Mechanics/CLI/items/1-weapon-xdmg.md).
...
Where these valves meet, at about waist height, is a cup-like depression, a hemispherical concavity with a central hole. The hole appears to fit the Second Key, but if it is inserted, the character so doing will take `dice:1d10|noform|noparens|avg|text(5)` (`1d10`) lightning damage. Anyone so foolish as to insert the First Key (from area 19) will take double that amount of damage.
...
If the *First Key* is inserted herein, the key explodes, and the individual doing so takes `dice:5d6|noform|noparens|avg|text(17)` (`5d6`) bludgeoning damage.
...
![](file://adventure/TftYP-ToH/007-totyp-07-09.webp#center)
