...
![](file://adventure/PotA/023-poa03-01.webp#center)
...
![](file://adventure/PotA/024-poa03-02.webp#center)
...
> [!gallery]
> ![Map 3.1: Feathergale Spire](file://adventure/PotA/025-03-01.webp#gallery)
> ![Player Version](file://adventure/PotA/026-poa5.webp#gallery)
...
Feathergale Knights keep their wealth locked in chests in their cells. Each knight also has `dice:2d10|noform|noparens|avg` (`2d10`) gp in mixed coinage and minor valuables.
...
Each knight's chest contains valuables worth `dice:d100|noform|noparens|avg` (`d100`) gp, and a feathered cloak.
...
Thurl's ring is worth 250 gp. Thurl also carries `dice:3d10|noform|noparens|avg` (`3d10`) gp and `dice:2d6|noform|noparens|avg` (`2d6`) pp in a pouch.
...
![](file://adventure/PotA/027-poa03-04.webp#center)
...
At the end of each minute spent searching for the manticore, have each group of hunters in the canyon roll a `dice:d20|noform|noparens|avg` (`d20`), adding 1 for each character or knight in the group. Any group that rolls a total of 18 or higher comes within 100 feet of the manticore. Any and all groups that find the manticore can engage it in battle.

If the knights find the manticore first, they blow their horns so that other groups can pinpoint the knights' location and join the battle in `dice:1d6|noform|noparens|avg` (`1d6`) rounds.
...
> [!gallery]
> ![Map 3.2: Sighing Valley](file://adventure/PotA/028-03-02.webp#gallery)
> ![Player Version](file://adventure/PotA/029-poa12.webp#gallery)
...
Each warrior in the keep carries `dice:2d10|noform|noparens|avg` (`2d10`)−2 gp in mixed coinage and minor valuables.
...
> [!gallery]
> ![Map 3.3: Rivergard Keep](file://adventure/PotA/030-03-03.webp#gallery)
> ![Player Version](file://adventure/PotA/031-poa9.webp#gallery)
...
If the characters kill some or all the gatehouse guards (areas K2 and K3) and then leave Rivergard, the dead guards are replaced by troops from areas K8 or K20. If those guards are killed too, it takes Grimjaw `dice:1d4+1|noform|noparens|avg` (`1d4 + 1`) days to get reinforcements and replace the gate guards. During this period, the front gate remains barred but unguarded.
...
Characters lingering in the castle yard are likely to encounter various denizens of Rivergard going about their business. Check for random encounters once per 5 minutes during the day or once per 15 minutes at night. Roll a `dice:d20|noform|noparens|avg` (`d20`); if the result is 18 or higher, a random encounter occurs. To determine the encounter, roll a `dice:d6|noform|noparens|avg` (`d6`) and consult the following table:

`dice: [](03-chapter-3-secret-of-the-sumber-hills.md#^encounter)`

| dice: d6 | Encounter |
|----------|-----------|
| 1 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [commoners](3-Mechanics/CLI/bestiary/humanoid/commoner-xmm.md) (see area K17) |
| 2 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) [bandits](3-Mechanics/CLI/bestiary/humanoid/bandit-xmm.md) (see area K8) |
| 3 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [Crushing Wave reavers](3-Mechanics/CLI/bestiary/humanoid/crushing-wave-reaver-pota.md) (see area K20) |
| 4 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [giant rats](3-Mechanics/CLI/bestiary/beast/giant-rat-xmm.md) |
...
![](file://adventure/PotA/032-poa03-07.webp#center)
...
Sacred Stone monks are ascetics and don't collect individual treasure. Their gilded tin masks are worthless. For the more worldly denizens of the monastery (such as the duergar), each carries `dice:2d10|noform|noparens|avg` (`2d10`) gp in mixed coinage and minor valuables.
...
> [!gallery]
> ![Map 3.4: Sacred Stone Monastery](file://adventure/PotA/033-03-04.webp#gallery)
> ![Player Version](file://adventure/PotA/034-poa10.webp#gallery)
...
![](file://adventure/PotA/035-03-06.webp#center)
...
If the characters kill this group of monsters and leave the dungeon without defeating [Hellenrae](3-Mechanics/CLI/bestiary/npc/hellenrae-pota.md), within `dice:1d3|noform|noparens|avg` (`1d3`) days she replaces these guards with six [duergar](3-Mechanics/CLI/bestiary/humanoid/spy-xmm.md). The duergar also arrive if the characters kill Jurth in a random encounter in the mine (area M22).
...
The monster in the western portion of the chamber is an [umber hulk](3-Mechanics/CLI/bestiary/monstrosity/umber-hulk-xmm.md). The cultists partially [blinded](3-Mechanics/CLI/rules/conditions.md#Blinded) the creature and removed its natural claws, so the hulk no longer has its Confusing Gaze power or the ability to burrow. However, the artificial blades increase its claw damage to `dice:2d8+5|noform|noparens|avg|text(14)` (`2d8 + 5`) slashing damage. The angry, trapped umber hulk eagerly attacks anything it can reach. If the characters goad it through the bars (for example, by attacking with ranged weapons or spells), the umber hulk goes berserk. It can use an action to attempt a DC 20 Strength check on each of its turns to wrench the cage door open.
...
The mines aren't heavily trafficked, but the cultists send their captives to work here and sometimes strange creatures wander in. Check for random encounters once per hour. Roll a `dice:d20|noform|noparens|avg` (`d20`); if the result is 18 or higher, a random encounter occurs. Roll a `dice:d6|noform|noparens|avg` (`d6`) for day encounters, or a `dice:d8|noform|noparens|avg` (`d8`) for night encounters:

| d6;d8 | Encounter |
|-------|-----------|
| 1 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) [commoners](3-Mechanics/CLI/bestiary/humanoid/commoner-xmm.md) (see area M19) |
| 2 | As per result 1, plus 1 [orog](3-Mechanics/CLI/bestiary/humanoid/berserker-xmm.md) (see area M18) |
| 3 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [Sacred Stone monks](3-Mechanics/CLI/bestiary/humanoid/sacred-stone-monk-pota.md) (see area M7) |
| 4 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [violet fungi](3-Mechanics/CLI/bestiary/plant/violet-fungus-xmm.md) |
| 5 | 1 [grick](3-Mechanics/CLI/bestiary/aberration/grick-xmm.md) |
| 6 | Jurth and `dice:1d2\|noform\|noparens\|avg` (`1d2`) [orogs](3-Mechanics/CLI/bestiary/humanoid/berserker-xmm.md) (see area M18) |
| 7–8 | `dice:1d3+1\|noform\|noparens\|avg` (`1d3 + 1`) [duergar](3-Mechanics/CLI/bestiary/humanoid/spy-xmm.md) (see area M5) |
^encounter
...
Most of the visitors here carry `dice:2d10|noform|noparens|avg` (`2d10`) gp in mixed coinage and minor valuables.
...
> [!gallery]
> ![Map 3.5: Scarlet Moon Hall](file://adventure/PotA/036-03-05.webp#gallery)
> ![Player Version](file://adventure/PotA/037-poa11.webp#gallery)
...
The area marked with lines creaks loudly whenever someone steps on it. At the start of every round, roll `dice:1d8|noform|noparens|avg` (`1d8`). If the result is equal to or lower than the number of creatures standing on the weak floor, it collapses, sending everyone falling 15 feet into area H5 or 35 feet into area H6.

#### H8. Elizar's Chamber

![](file://adventure/PotA/038-poa03-10.webp#center)
