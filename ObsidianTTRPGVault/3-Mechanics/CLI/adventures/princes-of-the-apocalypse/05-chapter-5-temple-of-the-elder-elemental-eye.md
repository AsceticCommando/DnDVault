...
![](file://adventure/PotA/053-poa05-01.webp#center)
...
> [!gallery]
> ![Map 5.1: Fane of the Eye](file://adventure/PotA/054-05-01.webp#gallery)
> ![Player Version](file://adventure/PotA/055-pam20_faneeye_dm-zf-0998-75774-1-011-0.webp#gallery)
...
A character unaware of the pit falls into it. The pit is 10 feet deep and contains a [rust monster](3-Mechanics/CLI/bestiary/monstrosity/rust-monster-xmm.md) that wandered into this cave after being driven out of area F9. The rust monster is heavily obscured by the mist and makes a surprise attack against a creature that falls into the pit, gaining advantage on its attack roll if its target is [prone](3-Mechanics/CLI/rules/conditions.md#Prone).
...
![](file://adventure/PotA/056-poa05-03.webp#center)
...
> [!note] 
> 
> **Burning Breath (Recharge 5-6).** The minotaur exhales a cloud of burning embers in a 15-foot cone. Each creature in that area must succeed on a DC 14 Dexterity saving throw, taking `dice:6d6|noform|noparens|avg|text(21)` (`6d6`) fire damage on a failed save, or half as much damage on a successful one.
^49a
...
![](file://adventure/PotA/057-poa05-04.webp#center)
...
A [glyph of warding](3-Mechanics/CLI/spells/glyph-of-warding-xphb.md) protects the sarcophagus. A character can find the glyph with a successful DC 18 Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) check. Triggered by any attempt to open the coffin, the glyph casts [Mordenkainen's sword](3-Mechanics/CLI/spells/mordenkainens-sword-xphb.md) (melee spell attack +10). The sword attacks any creature within 30 feet of the sarcophagus, starting with the triggering creature. It disappears after 1 minute.
...
> [!gallery]
> ![Map 5.2: Howling Caves](file://adventure/PotA/058-05-02.webp#gallery)
> ![Player Version](file://adventure/PotA/059-poa6.webp#gallery)
...
The shift to extreme heat, as described in "chapter 5" of the *Dungeon Master's Guide*, takes only a moment. When the heat begins, the ice cracks at the start of that round and is unable to hold any weight end of the round 3 rounds later. The lake is 12 feet deep and turns to boiling water. A creature that enters the water for the first time on its turn or starts its turn there takes `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) fire damage. In addition, steam renders the cave lightly obscured.
...
The chasm fills with wind, thunder, or lightning at random times. If one or more creatures are in the chasm, have them roll initiative. On initiative count 10, roll a `dice:d20|noform|noparens|avg` (`d20`) and consult the following table to determine what effect, if any, occurs in the chasm:
...
Thunder momentarily fills the chasm. Each creature in the chasm must make a DC 15 Constitution saving throw. On a failure, it takes `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) thunder damage and is [deafened](3-Mechanics/CLI/rules/conditions.md#Deafened) for 1 minute. On a success, the creature takes half damage and isn't [deafened](3-Mechanics/CLI/rules/conditions.md#Deafened).

##### Lightning

Lightning momentarily fills the chasm. Each creature in the chasm must make a DC 15 Dexterity saving throw, taking `dice:8d6|noform|noparens|avg|text(28)` (`8d6`) lightning damage on a failure, or half as much damage on a success.
...
Eating any other toadstool forces the eater to make a DC 15 Constitution saving throw after `dice:2d4|noform|noparens|avg` (`2d4`) minutes. Failure indicates the consumer is [poisoned](3-Mechanics/CLI/rules/conditions.md#Poisoned). A creature [poisoned](3-Mechanics/CLI/rules/conditions.md#Poisoned) in this way can repeat the saving throw at the end of each hour, ending the effect on a success.
...
Prisoners are kept here until they can be sacrificed to [Yan-C-Bin](3-Mechanics/CLI/bestiary/npc/yan-c-bin-pota.md) in area N19. Two [invisible](3-Mechanics/CLI/rules/conditions.md#Invisible) [barlguras](3-Mechanics/CLI/bestiary/fiend/barlgura-xmm.md) guard the prisoners. The demons lurk among the columns in the upper reaches of the cave, dropping down to attack intruders who dare free the prisoners. If they spot the characters, the two women warn characters of the "[invisible](3-Mechanics/CLI/rules/conditions.md#Invisible) demons" that guard them.
...
![](file://adventure/PotA/060-poa05-06.webp#center)
...
Whenever the characters explore a new area, roll a `dice:d6|noform|noparens|avg` (`d6`). On a roll of 1, choose one character to receive an image of the setting features (but not creatures) in area P4, P5, or P6. The aboleth living in the node is telepathically projecting these images to frighten trespassers and ultimately lure them to its lair.
...
Check for a random encounter every 10 minutes. Roll a `dice:d20|noform|noparens|avg` (`d20`) and consult the following table to determine what, if anything, the characters encounter:
...
| 2–3 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) [ice mephits](3-Mechanics/CLI/bestiary/elemental/ice-mephit-xmm.md) (see area P14) |
...
| 6 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) [swarms of quippers](3-Mechanics/CLI/bestiary/beast/swarm-of-piranhas-xmm.md) |
| 7–20 | None |
^encounter
...
> [!gallery]
> ![Map 5.3: The Plunging Torrents](file://adventure/PotA/061-05-03.webp#gallery)
> ![Player Version](file://adventure/PotA/062-poa8.webp#gallery)
...
> [!note] 
> 
> **Trident of Fish Command.** *Melee  or Ranged Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 5 ft. or range 20/60 ft., one target. *Hit*: `dice:1d6+2|noform|noparens|avg|text(5)` (`1d6 + 2`) piercing damage, or `dice:1d8+2|noform|noparens|avg|text(6)` (`1d8 + 2`) piercing damage if used with two hands to make a melee attack.
^521
...
Strong currents channel water from area P18 to these caves. In the center of each cave is a swirling vortex. Any creature drawn down into the vortex is teleported to a random body of water within the Plunging Torrents. Roll a `dice:d20|noform|noparens|avg` (`d20`); the number rolled determines of the location where the creature appears. For example, on a roll of 7, the creature teleports to the quipper pit in area P7.
...
![](file://adventure/PotA/063-poa05-08.webp#center)
...
Check for random encounters once every 30 minutes. Roll a `dice:d20|noform|noparens|avg` (`d20`) and consult the following table to determine what, if anything, the characters encounter:
...
| 2 | 1 [stonemelder](3-Mechanics/CLI/bestiary/humanoid/stonemelder-pota.md) and `dice:1d3+1\|noform\|noparens\|avg` (`1d3 + 1`) [Black Earth guards](3-Mechanics/CLI/bestiary/humanoid/black-earth-guard-pota.md) (see area G12) |
| 3 | `dice:1d2\|noform\|noparens\|avg` (`1d2`) [earth elementals](3-Mechanics/CLI/bestiary/elemental/earth-elemental-xmm.md) |
...
Stalactites within 30 feet of the party begin to drip like melting wax. By the time the change ceases, each stalactite has grown `dice:1d4|noform|noparens|avg` (`1d4`) feet longer. If a stalactite touches the opposite surface, it becomes a natural column.

> [!gallery]
> ![Map 5.4: The Black Geode](file://adventure/PotA/064-05-04.webp#gallery)
> ![Player Version](file://adventure/PotA/065-poa4.webp#gallery)
...
A thunderous noise emanates from the northeast tunnel, which is only 5 feet high and slopes gently upward to area G6. The tunnel ends at a wall of falling mud (see area G6). Any creature passing under the mud takes `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) bludgeoning damage.
...
The "secret" entrance to area G5 is a tunnel cloaked by a 6th-level [major image](3-Mechanics/CLI/spells/major-image-xphb.md) spell to look like a solid stone wall. Discovering its nature from a distance requires a DC 19 Intelligence ([Investigation](3-Mechanics/CLI/rules/skills.md#Investigation)) check, although direct interaction immediately reveals the illusion.
...
A 5-foot-high tunnel hidden behind the mud descends to area G4. A creature must pass through the mud to reach the tunnel, taking `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) bludgeoning damage from the falling mud.
...
A creature that enters the well of boiling mud for the first time on a turn or starts its turn there takes `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) fire damage. The mud is 5 feet deep and heated by a volcanic vent beneath the cave.
...
![](file://adventure/PotA/066-poa05-10.webp#center)
...
> [!gallery]
> ![Map 5.5: The Weeping Colossus](file://adventure/PotA/067-05-05.webp#gallery)
> ![Player Version](file://adventure/PotA/067-05-05-2.webp#gallery)
...
A creature takes `dice:6d10|noform|noparens|avg` (`6d10`) fire damage when it enters lava for the first time on a turn or when it ends its turn there.
...
For every 30 minutes the characters spend inside the Weeping Colossus, roll a `dice:d20|noform|noparens|avg` (`d20`) and consult the following table. Explanations of some of the encounters appear after the table. In the medium intensity zone, roll a `dice:d20|noform|noparens|avg` (`d20`) and a `dice:d4|noform|noparens|avg` (`d4`), subtracting the `dice:d4|noform|noparens|avg` (`d4`) from the `dice:d20|noform|noparens|avg` (`d20`) roll. Use a `dice:d6|noform|noparens|avg` (`d6`) instead of a `dice:d4|noform|noparens|avg` (`d4`) in the high intensity zone.
...
| 3 | `dice:1d8\|noform\|noparens\|avg` (`1d8`) [magmin](3-Mechanics/CLI/bestiary/elemental/magmin-xmm.md) |
| 4 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [salamanders](3-Mechanics/CLI/bestiary/elemental/salamander-xmm.md) (see area W7) |
| 5 | Crust break |
| 6 | Flame gout |
| 7 | Lava rain |
| 8 | Smoke cloud |
...
At the start of a round, lava droplets form on the ceiling and fall in a 20-foot-diameter area, dealing `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) fire damage to any creature that enters the area for the first time on a turn or starts its turn there. The drops ignite exposed flammable objects. The rain ends at the end of a round `dice:2d4|noform|noparens|avg` (`2d4`) rounds later.

##### Smoke Cloud

A cloud of smoke kicks up, filling an area 40 feet in diameter and causing that area to become lightly obscured. The smoke dissipates in `dice:1d4|noform|noparens|avg` (`1d4`) minutes unless a wind disperses it.
...
A [roper](3-Mechanics/CLI/bestiary/monstrosity/molten-magma-roper-pota.md) made of molten magma lurks in the middle of the lava pool, peeking up occasionally or rising to the sound of the gong. It has immunity to fire damage and vulnerability to cold damage, and its bite attack deals fire damage instead of piercing damage. A hit from the roper's tendril deals `dice:1d8|noform|noparens|avg|text(4)` (`1d8`) fire damage, and a creature takes the same damage each time it ends its turn [grappled](3-Mechanics/CLI/rules/conditions.md#Grappled) by the roper.
...
Due to the height of the room relative to the rest of the node, toxic fumes collect here. Whenever a creature starts its turn inside the room, it must make a DC 15 Constitution saving throw, taking `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) poison damage on a failed save, or half as much damage on a successful one. Holding one's breath doesn't negate the effects.
...
The ceiling here is 20 feet high. The fire is a ring-shaped [wall of fire](3-Mechanics/CLI/spells/wall-of-fire-xphb.md) spell made permanent (until dispelled) by [Vanifer](3-Mechanics/CLI/bestiary/npc/vanifer-pota.md). A creature that ends its turn in this room but outside the ring takes `dice:5d8|noform|noparens|avg` (`5d8`) fire damage. A creature takes the same damage when it enters the wall for the first time on a turn or ends its turn there.
...
![](file://adventure/PotA/068-poa05-12.webp#center)
...
![](file://adventure/PotA/069-poa05-13.webp#center)
