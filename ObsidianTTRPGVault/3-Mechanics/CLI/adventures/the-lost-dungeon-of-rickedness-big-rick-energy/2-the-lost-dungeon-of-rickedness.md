...
![](file://adventure/RMBRE/010-intro-3.webp#center)
...
The mouth spits out anything placed in it. If the characters are stupid enough to try destroying the mouth instead of answering, have any moron who attacks it get licked for `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) shame damage. That's right: shame. There's no resistance to shame.
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^fun-obstacle)`
...
For `dice:1d4|noform|noparens|avg` (`1d4`) rounds, the whole floor heaves like when you're completely hammered. For as long as the floor pitches, each creature that starts its turn standing on the floor must succeed on a DC 18 Dexterity ([Acrobatics](3-Mechanics/CLI/rules/skills.md#Acrobatics)) check or fall on its ass, [prone](3-Mechanics/CLI/rules/conditions.md#Prone).
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^effects)`

| dice: d4 | Effects |
|----------|---------|
| 1 | **Sleep**. The creature falls [unconscious](3-Mechanics/CLI/rules/conditions.md#Unconscious) for `dice:1d4\|noform\|noparens\|avg` (`1d4`) rounds, as if under the effect of a [sleep](3-Mechanics/CLI/spells/sleep-xphb.md) spell. |
| 2 | **Retching**. The creature is [poisoned](3-Mechanics/CLI/rules/conditions.md#Poisoned) for `dice:1d4\|noform\|noparens\|avg` (`1d4`) rounds. |
| 3 | **Laughter**. The creature is overcome by a fit of giggles (because farts are f***ing funny) and is [incapacitated](3-Mechanics/CLI/rules/conditions.md#Incapacitated) for `dice:1d4\|noform\|noparens\|avg` (`1d4`) rounds. |
| 4 | **Blindness**. Something spicy in that fart leaves the creature [blinded](3-Mechanics/CLI/rules/conditions.md#Blinded) for `dice:1d4\|noform\|noparens\|avg` (`1d4`) rounds. |
^effects

##### Punch Trap

A [goblin](3-Mechanics/CLI/bestiary/fey/goblin-warrior-xmm.md) emerges from a trapdoor and punches a random character right in the groin. The character needs to dodge with a successful DC 15 Dexterity saving throw or take `dice:1d4|noform|noparens|avg|text(2)` (`1d4`) bludgeoning damage. The goblin attacks once and then disappears back through the trapdoor, which immediately vanishes to leave no opportunity for counterattacks.
...
*Ranged Weapon Attack*: `dice:1d20+3|noform|noparens|text(+3)` to hit, range 100 ft., one target. *Hit*: `dice:1d8|noform|noparens|avg|text(4)` (`1d8`) slashing damage.

##### Tenderizer Trap

For the next `dice:1d4|noform|noparens|avg` (`1d4`) rounds, 5-foot-diameter chunks of the ceiling fall at regular intervals, pretty much making the room a giant game of whack-a-mole. Each round, a random character is targeted by a smashy bit, and must succeed on a DC 15 Dexterity saving throw or take `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) bludgeoning damage.
...
![](file://adventure/RMBRE/011-intro-4.webp#center)
...
*Melee Weapon Attack:* `dice:1d20+3|noform|noparens|text(+3)` to hit, reach 5 ft., one target. *Hit*: `dice:1d4+3|noform|noparens|avg|text(5)` (`1d4 + 3`) bludgeoning damage.
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^thingy)`
...
![](file://adventure/RMBRE/012-intro-5.webp#center)
...
The figure at the bottom of the pit is an [evil mage](3-Mechanics/CLI/bestiary/humanoid/evil-mage-lmop.md) named Glizzlegus. She was a member of the last group I ran through this dungeon. She rolled a 1 on her Dexterity saving throw and has 1 hit point remaining. When Glizzlegus spots the characters, she lifts her head. "Please, help me," she rasps. Sounds like she's got some lung damage. Oops!
...
![](file://adventure/RMBRE/013-intro-6.webp#center)
...
If a fight breaks out, the ghoul capitalizes on the distraction and gnaws through the ropes binding it. It takes `dice:1d4|noform|noparens|avg` (`1d4`) rounds for the ghoul to escape. When it does, it apologizes profusely for its immense hunger, then attacks the party. Undead, am I right? Jeez!
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^difference)`
...
![](file://adventure/RMBRE/014-intro-7.webp#center)
...
Just a warning, though: if someone gives a [Meeseeks](3-Mechanics/CLI/bestiary/humanoid/meeseeks-rmbre.md) a purpose it can't fulfill, or if a [Meeseeks](3-Mechanics/CLI/bestiary/humanoid/meeseeks-rmbre.md) is prevented from fulfilling its purpose, it does go sort of insane, becoming hostile toward its summoner after `dice:1d4|noform|noparens|avg` (`1d4`) hours.

While the box is in the room, it can summon as many [Meeseeks](3-Mechanics/CLI/bestiary/humanoid/meeseeks-rmbre.md) as you like. It's also possible to take the box. If it's used outside the room, though, roll a `dice:d20|noform|noparens|avg` (`d20`) and add the number of [Meeseeks](3-Mechanics/CLI/bestiary/humanoid/meeseeks-rmbre.md) that have already been summoned. If the total is higher than 20, the [Meeseeks](3-Mechanics/CLI/bestiary/humanoid/meeseeks-rmbre.md) box implodes and makes a sad whining sound, destroying it.
...
![](file://adventure/RMBRE/015-intro-8.webp#center)
...
When a character fires the churd cannon in area 10, have the character's player roll a `dice:d20|noform|noparens|avg` (`d20`). On any roll other than a 1, the cannon destroys the Butt Trap in a blinding flash. See area 10 for details on what happens afterward.
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^wondrous-ware)`
...
![](file://adventure/RMBRE/016-intro-9.webp#center)
...
![](file://adventure/RMBRE/017-intro-10.webp#center)
...
![](file://adventure/RMBRE/018-intro-11.webp#center)
...
![](file://adventure/RMBRE/019-intro-12.webp#center)
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^minor-doom)`
...
| 2 | An orange tabby cat appears. Aw, it likes you! Every `dice:1d4\|noform\|noparens\|avg` (`1d4`) minutes, the cat grows another leg out of a random body part. Enjoy your upcoming nightmares. |
...
| 5 | Everyone standing in the room is teleported onto the ceiling, then falls back to the floor, taking `dice:1d6\|noform\|noparens\|avg\|text(3)` (`1d6`) bludgeoning damage. Gravity! She's a fickle mistress. |
...
![](file://adventure/RMBRE/020-intro-13.webp#center)
...
If one or more of the party members have previously lost their butts, Zorg and the other members of the Order of the Buttless begin to get excited, asking repeatedly whether this might be the Buttless Messiah (or messiahs) that the prophecy foretold. To dispel everyone's curiosity, Zorg produces the *Book of the Buttless*, an enviably pert tome filled with prophecies and star charts that predict the coming of the Buttless Messiah. To determine the meanings of omens that could herald the messiah, roll a `dice:d6|noform|noparens|avg` (`d6`) and consult the Buttless Prophecy table.

#### Buttless Prophecy

`dice: [](2-the-lost-dungeon-of-rickedness.md#^the-truth)`
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^regarding-insight)`
...
![](file://adventure/RMBRE/021-intro-14.webp#center)
...
*Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 5 ft., one target. *Hit*: `dice:1d8+2|noform|noparens|avg|text(6)` (`1d8 + 2`) bludgeoning damage.
...
*Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit*: `dice:1d8+3|noform|noparens|avg|text(7)` (`1d8 + 3`) bludgeoning damage.
...
*Melee Weapon Attack:* `dice:1d20+6|noform|noparens|text(+6)` to hit, reach 5 ft., one target. *Hit*: `dice:1d8+4|noform|noparens|avg|text(8)` (`1d8 + 4`) bludgeoning damage.
...
Like, a thousand percent of injuries in the home happen in the kitchen. So any restaurant is pretty much a death trap where you're expected to tip. If the characters want to find cooking oil or bottles of alcohol and set them aflame, go for it! Doing so nets them `dice:2d6|noform|noparens|avg` (`2d6`) improvised ranged weapons. Likewise, if the characters want to create walls and choke points from dining tables, great! If they want to pull a giant pizza spatula off the wall and use it as a [maul](3-Mechanics/CLI/items/maul-xphb.md), beautiful! Remember, the customer is always right!
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^treasure)`
...
`dice: [](2-the-lost-dungeon-of-rickedness.md#^instrument)`
...
![](file://adventure/RMBRE/022-intro-15.webp#center)
...
![](file://adventure/RMBRE/023-intro-16.webp#center)
