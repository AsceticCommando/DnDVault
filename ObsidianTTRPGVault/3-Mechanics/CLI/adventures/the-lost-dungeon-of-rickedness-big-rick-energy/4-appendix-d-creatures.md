...
Skills in a creature's stat block are shown with the total modifier—the creature's ability modifier plus its proficiency bonus. If a creature's stat block says "Stealth +6," roll a `dice:d20|noform|noparens|avg` (`d20`) and add 6 when the creature makes an ability check using [Stealth](3-Mechanics/CLI/rules/skills.md#Stealth).
...
Any damage or other effects that occur as a result of an attack hitting a target are described here. As the DM, you can take average damage or roll the damage; for this reason, both the average damage and the die expression are presented. For example, a monster might deal `dice:1d8|noform|noparens|avg|text(4)` (`1d8`) slashing damage with its [longsword](3-Mechanics/CLI/items/longsword-xphb.md). That notation means you can have the monster deal 4 damage, or you can roll `dice:1d8|noform|noparens|avg` (`1d8`) to determine the damage.
