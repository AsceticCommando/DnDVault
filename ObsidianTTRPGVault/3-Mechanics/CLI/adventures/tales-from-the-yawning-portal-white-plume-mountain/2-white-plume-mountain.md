...
> 
> ![](file://adventure/TftYP-WPM/000-totyp-04-book.webp#center)
^022
...
![Map 4.1: Cutaway View](file://adventure/TftYP-WPM/001-wpm01.webp#center)

### Random Encounters

Check once every 10 minutes for random encounters by rolling a `dice:d12|noform|noparens|avg` (`d12`); an encounter occurs on a roll of 1. If an encounter is indicated, roll a `dice:d6|noform|noparens|avg` (`d6`) and refer to the table below.
...
| 2 | `dice:1d4+1\|noform\|noparens\|avg` (`1d4 + 1`) [bugbears](3-Mechanics/CLI/bestiary/fey/bugbear-warrior-xmm.md) |
...
| 5 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [ogres](3-Mechanics/CLI/bestiary/giant/ogre-xmm.md) |
| 6 | `dice:1d2\|noform\|noparens\|avg` (`1d2`) [wights](3-Mechanics/CLI/bestiary/undead/wight-xmm.md) |
^encounter
...
> [!gallery]
> ![Map 4.2: White Plume Mountain](file://adventure/TftYP-WPM/002-wpm02.webp#gallery)
> ![Player Version](file://adventure/TftYP-WPM/003-wpm01.webp#gallery)
...
![](file://adventure/TftYP-WPM/003-totyp-04-05.webp#center)
...
| Location | Damage | DC |
|----------|--------|----|
| Adjacent to geyser | 27 (`dice:5d10\|noform\|noparens\|avg` (`5d10`)) fire damage | 14 |
| One disk away | 22 (`dice:4d10\|noform\|noparens\|avg` (`4d10`)) fire damage | 13 |
| Two disks away | 16 (`dice:3d10\|noform\|noparens\|avg` (`3d10`)) fire damage | 12 |
| Three disks away | 11 (`dice:2d10\|noform\|noparens\|avg` (`2d10`)) fire damage | 11 |
| Four disks away | 5 (`dice:1d10\|noform\|noparens\|avg` (`1d10`)) fire damage | 10 |
| Anywhere else in the area | 3 (`dice:1d6\|noform\|noparens\|avg` (`1d6`)) fire damage | — |
^location-damage-dc

Any creature that falls into the boiling mud takes `dice:8d10|noform|noparens|avg|text(44)` (`8d10`) fire damage at the start of each of its turns for as long as it remains in the mud.

![Map 4.3: Geysers and Chains](file://adventure/TftYP-WPM/004-wpm03.webp#center)
...
![](file://adventure/TftYP-WPM/005-totyp-04-06.webp#center)
...
The oil burns for `dice:2d4|noform|noparens|avg` (`2d4`) rounds if the fire is not extinguished. On the first round, the burning oil deals `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) fire damage to any creature that enters the spinning cylinder or starts its turn inside the cylinder. On later rounds, it deals `dice:1d4|noform|noparens|avg|text(2)` (`1d4`) fire damage to such creatures.
...
> [!note] 
> 
> **Spellcasting.** [Snarla](3-Mechanics/CLI/bestiary/npc/snarla-tftyp.md) is a 6th-level spellcaster. Her spellcasting ability is Intelligence (spell save DC 14, `dice:1d20+6|noform|noparens|text(+6)` to hit with spell attacks). She has the following wizard spells prepared:
> 
...
The room has a couple of benches and a table, upon which are a large candlestick (worth 10 gp) and [Snarla](3-Mechanics/CLI/bestiary/npc/snarla-tftyp.md)'s spellbook. The book contains the spells that she has prepared and no others. It is protected by an explosive runes [glyph of warding](3-Mechanics/CLI/spells/glyph-of-warding-xphb.md) that deals `dice:5d6|noform|noparens|avg` (`5d6`) fire damage.
...
Any creature that enters the boiling lake takes `dice:8d10|noform|noparens|avg|text(44)` (`8d10`) fire damage immediately and again at the start of each of its turns for as long as it remains in the lake.

![Map 4.4: Boiling Lake](file://adventure/TftYP-WPM/006-wpm04.webp#center)
...
The protective skin is soft, resilient, and uncomfortably warm to the touch. Under any pressure it immediately becomes taut, and any character unwise enough to puncture it with a piercing weapon will cause a stream of scalding water to rush into the corridor, hopefully burning the idiot who made the hole for `dice:1d4|noform|noparens|avg|text(2)` (`1d4`) fire damage). Thereafter, any creature that enters the space with this stream of scalding water or that starts its turn there takes that damage. The skin will not "heal" once it is compromised. Major damage to the skin, as from a slash with a sword or an axe, will collapse the field like a deflating balloon in `dice:1d6|noform|noparens|avg` (`1d6`) rounds.
...
- It has `dice:14d12+70|noform|noparens|avg|text(161)` (`14d12 + 70`) hit points.  
...
> [!note] 
> 
> **Claw.** *Melee Weapon Attack:* `dice:1d20+9|noform|noparens|text(+9)` to hit, reach 10 ft., one target. Hit: `dice:4d10+5|noform|noparens|avg|text(27)` (`4d10 + 5`) bludgeoning damage, and the target is [grappled](3-Mechanics/CLI/rules/conditions.md#Grappled) (escape DC 14). The crab has two claws, each of which can grapple only one target.
^04d
...
![](file://adventure/TftYP-WPM/007-totyp-04-08.webp#center)
...
A character in metal armor who tries to move through this corridor would take damage as follows: `dice:1d8|noform|noparens|avg|text(4)` (`1d8`) fire damage at 40 feet, another `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) fire damage at 50 feet, an additional `dice:3d10|noform|noparens|avg|text(16)` (`3d10`) fire damage at 60 feet, and another `dice:4d10|noform|noparens|avg|text(22)` (`4d10`) fire damage at 70 feet.
...
The openings in the floor are pits, each 5 feet wide and 10 feet deep. Their bottoms are lined with rusty, razor-like blades. Anyone who falls in takes `dice:1d12|noform|noparens|avg|text(6)` (`1d12`) damage and must succeed on a DC 15 Constitution saving throw or contract a disease called super-tetanus (see below).
...
![Map 4.5: Aquarium and Prison](file://adventure/TftYP-WPM/008-wpm05.webp#center)
...
A safe is set in the north wall opposite the door on the bottom tier. If it is opened incorrectly (that is, the trap not removed), a vibration device in the wall is triggered that will shatter the glass walls in this area in `dice:1d6|noform|noparens|avg` (`1d6`) rounds. The safe contains 6,000 sp and one piece of jewelry worth 3,000 gp.

![](file://adventure/TftYP-WPM/009-totyp-04-10.webp#center)
...
![](file://adventure/TftYP-WPM/010-totyp-04-11.webp#center)
