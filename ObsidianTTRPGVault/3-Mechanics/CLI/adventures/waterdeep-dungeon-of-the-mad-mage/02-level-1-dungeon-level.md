...
![](file://adventure/WDMM/001-101.webp#center)
...
> [!gallery]
> ![Map 1](file://adventure/WDMM/002-102.webp#gallery)
> ![Player Version](file://adventure/WDMM/003-103.webp#gallery)
...
> [!note] 
> 
> **Bandits**. Unless they have been encountered elsewhere, four members of the Undertakers sit around a decrepit wooden table near the door: Uktarl Krannoc (NE human [bandit captain](3-Mechanics/CLI/bestiary/humanoid/bandit-captain-xmm.md) with [Performance](3-Mechanics/CLI/rules/skills.md#Performance) [Performance](3-Mechanics/CLI/rules/skills.md#Performance) (`dice:1d20+4|noform|noparens|text(+4)`)), two human [bandits](3-Mechanics/CLI/bestiary/humanoid/bandit-xmm.md), and a [doppelganger](3-Mechanics/CLI/bestiary/monstrosity/doppelganger-xmm.md), all disguised as vampires. They play cards using a marked deck that Uktarl carries around, and each has a stack of coins on the table.
> 
...
The throne's serpent armrests magically animate and bite anyone who sits on the throne or raises the seat's lid. Each snake makes a single melee weapon attack (`dice:1d20+8|noform|noparens|text(+8)` to hit) and deals `dice:1d4+1|noform|noparens|avg|text(3)` (`1d4 + 1`) piercing damage on a hit. Any creature bitten by a snake must make a DC 13 Constitution saving throw, taking `dice:7d6|noform|noparens|avg|text(24)` (`7d6`) poison damage on a failed save, or half as much damage on a successful one. After each snake makes one attack, the armrests revert to their inanimate state until triggered again.

#### 8b. Harria's Room

> [!note] 
> 
> **Bandit Captain.** Unless awakened by loud noises, Harria Valashtar (NE human [bandit captain](3-Mechanics/CLI/bestiary/humanoid/bandit-captain-xmm.md) with [Performance](3-Mechanics/CLI/rules/skills.md#Performance) [Performance](3-Mechanics/CLI/rules/skills.md#Performance) (`dice:1d20+4|noform|noparens|text(+4)`)) sleeps on a cot against the south wall. She is disguised as a vampire but doesn't wear her fangs while sleeping.
> 
...
One of Halaster's magic gates once stood between the pillars but was destroyed during the Spellplague. The gate's destruction left behind a lingering aura of wild magic that can be perceived with a [detect magic](3-Mechanics/CLI/spells/detect-magic-xphb.md) spell. When a creature passes between the pillars, roll a `dice:d12|noform|noparens|avg` (`d12`) and consult the following table to determine what happens:
...
The [mimic](3-Mechanics/CLI/bestiary/monstrosity/large-mimic-wdmm.md) is a Large specimen with `dice:10d10+20|noform|noparens|avg|text(75)` (`10d10 + 20`) hit points. It knocked over the original statue and pushed the broken pieces into the back of the alcove, assuming the shape of the statue afterward.
