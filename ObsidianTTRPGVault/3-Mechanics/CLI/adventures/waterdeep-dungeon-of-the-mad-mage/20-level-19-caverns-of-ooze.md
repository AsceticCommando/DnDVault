...
![](file://adventure/WDMM/060-19-01.webp#center)
...
- As an action, it can make two attacks with its pseudopods (`dice:1d20+7|noform|noparens|text(+7)` to hit). On a hit, a pseudopod deals `dice:6d6|noform|noparens|avg|text(21)` (`6d6`) acid damage, or `dice:12d6|noform|noparens|avg|text(42)` (`12d6`) acid damage while the ooze is enlarged.  
...
The pudding's spellcasting ability is Intelligence (spell save DC 12, `dice:1d20+4|noform|noparens|text(+4)` to hit with spell attacks). It can cast the following spells, requiring no components:
...
> [!gallery]
> ![Map 19](file://adventure/WDMM/061-19-02.webp#gallery)
> ![Player Version](file://adventure/WDMM/062-19-03.webp#gallery)
...
![](file://adventure/WDMM/063-19-04.webp#center)
...
![](file://adventure/WDMM/064-19-05.webp#center)
...
As an action, the animated statue makes a single melee weapon attack (`dice:1d20+10|noform|noparens|text(+10)` to hit) with its claws. On a hit, the claws deal `dice:5d6+5|noform|noparens|avg|text(22)` (`5d6 + 5`) slashing damage to the target, and any nonmagical armor worn by the target is destroyed.
...
A ballista is a Large object with AC 15, 50 hit points, and immunity to poison and psychic damage. Before it can be fired, a ballista must be loaded and aimed. (The ballistae are loaded when the characters first arrive.) It takes one action to load the weapon, one action to aim it, and one action to fire it (`dice:1d20+6|noform|noparens|text(+6)` to hit, range 120/480 ft., one target). On a hit, a ballista bolt deals `dice:3d10|noform|noparens|avg|text(16)` (`3d10`) piercing damage.
...
This cabin is furnished like area 13e and also contains a dead orog lying face down on the floor. Examination of the corpse reveals that the orog's skull was cracked open and its brain sucked out. Lying at the orog's feet is a [greataxe](3-Mechanics/CLI/items/greataxe-xphb.md).
