...
![](file://adventure/HotDQ/024-tod-05-01.webp#center)
...
Nothing much needs to happen on the trip north of Waterdeep. You can narrate through it quickly. At your option, use the random encounters below to spice up the journey. Roll a `dice:d20|noform|noparens|avg` (`d20`) each day and use the indicated encounter. Add 2 to the roll on days 8, 9, and 10.

Remember that the characters are just part of the convoy escort. The monsters listed on the Encounters North of Waterdeep table are those the characters must fight. At the same time, NPC members of the escort are dealing with additional monsters not listed here. Assume that monsters and the travelers are being alert before these encounters. After each encounter, roll `dice:1d4-2|noform|noparens|avg` (`1d4-2`). The result is the number of NPC escorts killed elsewhere in the battle.
...
> [!gallery]
> ![Map 5.1: Carnath Roadhouse](file://adventure/HotDQ/025-map-5-1-carnath-roadhouse.webp#gallery)
> ![Player Version](file://adventure/HotDQ/026-map-5-1-carnath-roadhouse-player.webp#gallery)
