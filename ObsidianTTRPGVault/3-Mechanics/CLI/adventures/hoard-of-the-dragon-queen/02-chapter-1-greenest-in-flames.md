...
![](file://adventure/HotDQ/006-tod-01-01.webp#center)
...
> [!gallery]
> ![Map 1.1: Greenest](file://adventure/HotDQ/007-map-1-1-greenest.webp#gallery)
> ![Player Version](file://adventure/HotDQ/008-map-1-1-greenest-player.webp#gallery)

### Important Characters

#### Governor Nighthill

![Tarbaw Nighthill](file://adventure/HotDQ/009-tod-01-02.webp#center)
...
If characters use cover and stealth to avoid encounters, have each character attempt a DC 10 Dexterity ([Stealth](3-Mechanics/CLI/rules/skills.md#Stealth)) check. For every two individual checks that fail, the characters have one encounter on the way to their destination. Roll a `dice:d8|noform|noparens|avg` (`d8`) on the Chapter 1 Encounters table to determine each encounter. If characters use the stream bed for cover for most of the trip, these characters have advantage on their Dexterity checks.

If characters don't use cover and stealth to avoid encounters, roll a `dice:d8|noform|noparens|avg` (`d8`) for every 100 feet they move in town. If the roll is 4 or lower, they didn't attract attention with that move. If the roll is 5 or higher, they run into something; roll a `dice:d8|noform|noparens|avg` (`d8`) again and check the Chapter 1 Encounters table to see what the characters meet.
...
| 7 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) townsfolk being hunted by raiders (roll a `dice:d6\|noform\|noparens\|avg` (`d6`) to determine the raiding group) |
| 8 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) townsfolk hiding |
^encounter
...
The woman is [Linan Swift](3-Mechanics/CLI/bestiary/npc/linan-swift-hotdq.md), and her husband is Cuth. Linan is a [commoner](3-Mechanics/CLI/bestiary/humanoid/commoner-xmm.md) but with 8 hit points. Her attack with the spear is `dice:1d20+2|noform|noparens|text(+2)` to hit for `dice:1d6|noform|noparens|avg` (`1d6`) piercing damage. Her husband is down to 2 hit points from an earlier fight. The children move at speed 20. They can be carried, but a character carrying a child has disadvantage on attack rolls and cannot wield a two-handed weapon.
...
To reach the keep, the characters must make it past three groups of raiders. A group consists of `dice:1d6|noform|noparens|avg` (`1d6`) [kobolds](3-Mechanics/CLI/bestiary/dragon/kobold-warrior-xmm.md) and `dice:1d4|noform|noparens|avg` (`1d4`) [cultists](3-Mechanics/CLI/bestiary/humanoid/cultist-xmm.md). If the group contains six kobolds, one is a [winged kobold (urd)](3-Mechanics/CLI/bestiary/dragon/winged-kobold-xmm.md).
...
Each time the characters retreat from an enemy group to avoid it, they run into `dice:1d6|noform|noparens|avg` (`1d6`) more townsfolk who are trying to reach the keep. For every four additional townsfolk in tow, the group must move past one more enemy group to reach the keep.
...
Award standard XP for defeated foes. Aside from that, the chief reward for this mission is the tunnel itself. As long as it remains secret, characters can use it to enter and exit the keep safely. Each time they use the tunnel exit, roll a `dice:d6|noform|noparens|avg` (`d6`). On a 1, raiders see and attack the characters (use the Chapter 1 Encounters table). On a 2, they are seen but not attacked. Instead, the raiders set an ambush and attack the next time the characters return to the tunnel exit.
