...
![](file://adventure/HotDQ/016-tod-03-01.webp#center)
...
Kobolds move freely through the caverns, going about their business. Characters have a chance to randomly encounter these wandering kobolds. Roll a `dice:d6|noform|noparens|avg` (`d6`) every time characters enter or re-enter areas 3 to 10. On a roll of 1, monsters are present. Roll on the table to determine what they meet. These randomly encountered monsters are added to any creatures mentioned in the area description. Be sure to make this roll before characters enter the area, because both groups have a chance to gain surprise.
...
> [!gallery]
> ![Map 3.1: Dragon Hatchery](file://adventure/HotDQ/017-map-3-1-dragon-hatchery.webp#gallery)
> ![Player Version](file://adventure/HotDQ/018-map-3-1-dragon-hatchery-player.webp#gallery)
...
Nothing distinguishes the paths to casual inspection. The path on the right is flanked by four [violet fungi](3-Mechanics/CLI/bestiary/plant/violet-fungus-xmm.md), while the path on the left is free of these dangerous growths. The violet fungi can be spotted among all the other mushrooms with a successful DC 15 Intelligence ([Nature](3-Mechanics/CLI/rules/skills.md#Nature)) check, but only from the base of the steps, not from atop the ledge. Likewise, a careful inspection of the path reveals that only the left trail sees heavy use.
...
The top step is rigged to drop a portion of the ceiling in area 7. As each character enters the staircase, roll any die. On an odd roll, the character steps in the wrong spot and triggers the trap. The ceiling collapses above the next character in line (the one behind the character who triggered the trap). That character takes `dice:4d4|noform|noparens|avg` (`4d4`) bludgeoning damage from falling rocks; the damage is halved if the character makes a successful DC 15 Dexterity saving throw. Every character within 5 feet takes `dice:2d4|noform|noparens|avg` (`2d4`) bludgeoning damage, or half that with a successful saving throw.
...
Everyone in the chamber must succeed on a DC 10 Dexterity saving throw or take `dice:2d6|noform|noparens|avg` (`2d6`) acid damage from the acid sprayed onto them (half as much damage on a successful saving throw). The real danger from the acid is not to the characters' skin, however, but to their lungs. Everyone must also succeed on a DC 10 Constitution saving throw or take an extra `dice:2d8|noform|noparens|avg` (`2d8`) acid damage. Players who declare immediately that their characters are covering their faces and getting out of the chamber as quickly as possible, and are trying to not inhale the vapor, have advantage on their Constitution saving throws.
...
![Guard Drake](file://adventure/HotDQ/019-tod-03-02.webp#center)
...
Each creature within 10 feet of the bomb's target point must succeed on a DC 11 Dexterity saving throw or take `dice:1d8|noform|noparens|avg|text(4)` (`1d8`) fire damage (half as much damage on a successful saving throw).
