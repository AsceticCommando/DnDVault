...
> 
> ![](file://adventure/TftYP-TSC/000-totyp-01-book.webp#center)
> 
...
> 
> **Random Treasure.** Each regular kobold and goblin is likely to have `dice:2d10|noform|noparens|avg` (`2d10`) sp, perhaps in the form of various coins and crude jewelry.
...
> [!gallery]
> ![Map 1.1: Fortress Level](file://adventure/TftYP-TSC/001-totyp-01-02.webp#gallery)
> ![Player Version](file://adventure/TftYP-TSC/002-tsc01.webp#gallery)
...
Each time a creature falls into a cavity, the noise is 10 percent likely to draw `dice:1d4|noform|noparens|avg` (`1d4`) [giant rats](3-Mechanics/CLI/bestiary/beast/giant-rat-xmm.md) from the dozens that infest the rubble field. Attracted rats move carefully and stealthily through the rubble to attack.
...
Each skeleton has twenty arrows, `dice:2d10|noform|noparens|avg` (`2d10`) sp, `dice:1d10|noform|noparens|avg` (`1d10`) gp, and one [+1 arrow](3-Mechanics/CLI/items/1-ammunition-xdmg.md) in a special socket on the quiver. Because the skeletons are mindless, they don't use the magical arrows.
...
![](file://adventure/TftYP-TSC/002-totyp-01-03.webp#center)
...
The pit is 10 feet deep, and its bottom is filled with spikes that deal `dice:2d10|noform|noparens|avg|text(11)` (`2d10`) piercing damage to a creature that falls in, as well as falling damage. The walls of the pit are rough, and they offer handholds to climbers. Area 12 begins on its far side.
...
> 
> In areas 13 and beyond, monsters move through the halls of the Sunless Citadel. For every 12 hours the characters spend in the dungeon, roll a `dice:d20|noform|noparens|avg` (`d20`) and refer to the following table if the characters are in an accessible hallway or room at that time.
> 
> `dice: [](2-the-sunless-citadel.md#^encounter)`
> 
> | dice: d20 | Encounter |
> |-----------|-----------|
> | 1 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) [kobolds](3-Mechanics/CLI/bestiary/dragon/kobold-warrior-xmm.md) |
> | 2 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) [giant rats](3-Mechanics/CLI/bestiary/beast/giant-rat-xmm.md) |
> | 3 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) [goblins](3-Mechanics/CLI/bestiary/fey/goblin-warrior-xmm.md) |
> | 4 | `dice:1d2\|noform\|noparens\|avg` (`1d2`) [hobgoblins](3-Mechanics/CLI/bestiary/fey/hobgoblin-warrior-xmm.md) |
> | 5 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) [skeletons](3-Mechanics/CLI/bestiary/undead/skeleton-xmm.md) |
> | 6 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) [twig blights](3-Mechanics/CLI/bestiary/plant/twig-blight-xmm.md) |
> | 7–20 | No encounter |
> ^encounter
^random-encounters
...
![](file://adventure/TftYP-TSC/003-totyp-01-04.webp#center)
...
Four [goblins](3-Mechanics/CLI/bestiary/fey/goblin-warrior-xmm.md) are chained here, taken prisoner during the course of several skirmishes. Every so often, the kobolds remove a goblin from the room and ransom it back to the goblins (for a price of `dice:2d10|noform|noparens|avg` (`2d10`) sp). Those who can't be ransomed are bound for the kobold cook pot. The goblins whine and cower pathetically if any characters enter the room. A character who succeeds on a DC 15 Dexterity check using thieves' tools can open the lock on the chains, or the chains can be broken (Strength DC 20) or sundered (AC 15, 60 hit points).
...
![](file://adventure/TftYP-TSC/004-totyp-01-05.webp#center)
...
If someone tries to open the door without first deactivating the lock, a scythe blade springs forth, targeting the area immediately in front of the door: `dice:1d20+5|noform|noparens|text(+5)` to hit, `dice:1d8|noform|noparens|avg|text(4)` (`1d8`) slashing damage.
...
The occupied rat nests contain shiny bits collected by their current owners. Poking through each nest turns up `dice:2d6|noform|noparens|avg` (`2d6`)−2 sp, `dice:1d6|noform|noparens|avg` (`1d6`)−1 gp, and `dice:1d4|noform|noparens|avg` (`1d4`)−1 gems (worth 5 gp each).
...
Three [giant rats](3-Mechanics/CLI/bestiary/beast/giant-rat-xmm.md) occupy the area, along with a monstrously swollen female diseased giant rat (Medium [6 feet long], 16 hit points, `dice:1d20+5|noform|noparens|text(+5)` to hit, challenge rating 1/4 [50 XP]) that the goblins call [Guthash](3-Mechanics/CLI/bestiary/npc/guthash-tftyp.md) (Bloated One). The giant rats that hunt in the Sunless Citadel all descend from her. These creatures attack and attempt to eat all trespassers.
...
![](file://adventure/TftYP-TSC/005-totyp-01-06.webp#center)
...
![](file://adventure/TftYP-TSC/006-totyp-01-07.webp#center)
...
The chief of the Durbuluk tribe is [Durnn](3-Mechanics/CLI/bestiary/npc/durnn-tftyp.md), a [hobgoblin](3-Mechanics/CLI/bestiary/fey/hobgoblin-warrior-xmm.md). He wears splint armor (AC 19) and has 18 hit points, a `dice:1d20+4|noform|noparens|text(+4)` bonus to hit, a +1 bonus to damage, and Strength 15 (+2). These changes raise his challenge rating to 1 (200 XP). When not in combat, [Durnn](3-Mechanics/CLI/bestiary/npc/durnn-tftyp.md) sits on the throne, and three other [hobgoblins](3-Mechanics/CLI/bestiary/fey/hobgoblin-warrior-xmm.md) stand nearby or lounge on stone benches.

The shaman [Grenl](3-Mechanics/CLI/bestiary/npc/grenl-tftyp.md) advises the chief. She is a [goblin](3-Mechanics/CLI/bestiary/fey/goblin-warrior-xmm.md) with `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) hit points, a Wisdom of 13 (+1), and the following trait:

##### Spellcasting

[Grenl](3-Mechanics/CLI/bestiary/npc/grenl-tftyp.md) is a 1st-level spellcaster. Her spellcasting ability is Wisdom (spell save DC 11, `dice:1d20+3|noform|noparens|text(+3)` to hit with spell attacks). She has the following cleric spells prepared:
...
When the trap is triggered, the needle extends 3 inches straight out from the lock. A creature within striking distance takes 1 piercing damage and must succeed on a DC 10 Constitution saving throw or take `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) poison damage.
...
> [!gallery]
> ![Map 1.2: Grove Level](file://adventure/TftYP-TSC/007-totyp-01-08.webp#gallery)
> ![Player Version](file://adventure/TftYP-TSC/009-tsc02.webp#gallery)
...
Two robed [skeletons](3-Mechanics/CLI/bestiary/undead/skeleton-xmm.md) (AC 12) serve Belak by tending a fungus garden, using shovels and a rusted wheelbarrow to turn the soil and spread compost. In addition, two mature [twig blights](3-Mechanics/CLI/bestiary/plant/twig-blight-xmm.md) are rooted in the garden. The twig blights ambush intruders that aren't goblinoids, and the skeletons attack (with shovels, `dice:1d6|noform|noparens|avg` (`1d6`) bludgeoning damage) anything that the twig blights target.
...
![](file://adventure/TftYP-TSC/008-totyp-01-09.webp#center)
...
A [bugbear gardener](3-Mechanics/CLI/bestiary/fey/bugbear-warrior-xmm.md) roams both galleries, tending the grasses for Belak. She is armed with a sickle-bladed glaive; she has a reach of 10 feet and deals `dice:2d10+2|noform|noparens|avg` (`2d10 + 2`) slashing damage with it.
...
Ten [twig blights](3-Mechanics/CLI/bestiary/plant/twig-blight-xmm.md), less any that were dispatched in area 54, are spread about the grove. After moving 50 feet into the cavern, the characters come upon `dice:1d4|noform|noparens|avg` (`1d4`) of the hostile creatures. Any significant noise, such as from combat or clearing the briars, also attracts `dice:1d4|noform|noparens|avg` (`1d4`)−1 twig blights (minimum 0) each round.
...
> 
> **Fruit.** Two magical fruits (evocation school) ripen on the tree each year. Each fruit resembles an apple, but it slowly extrudes from the tree like a cyst. Up to a week before the summer solstice, a ruby red fruit grows. Someone other than a construct who eats a quarter or more of it regains 70 hit points. In addition, any blindness, deafness, and diseases affecting the eater end. Up to a week before the winter solstice, a sickly white fruit grows. Someone who eats a quarter or more of it takes 70 necrotic damage. Each fruit has `dice:1d4|noform|noparens|avg` (`1d4`) seeds that, if planted, grow leafless woody shrubs that animate as twig blights after one year.
> 
...
During a battle, if any twig blights remain in area 55, `dice:1d4|noform|noparens|avg` (`1d4`)−1 of them (minimum 0) enter this area each round.
