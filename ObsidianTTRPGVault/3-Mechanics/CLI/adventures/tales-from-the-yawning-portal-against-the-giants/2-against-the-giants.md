...
> 
> ![](file://adventure/TftYP-AtG/000-totyp-06-book.webp#center)
> 
...
| 04–17 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) small boulders |
| 18–19 | Battered tin or pewter bowl and spoon |
| 20–21 | Various and sundry bent brass items |
| 22–23 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) large iron caltrops |
...
| 99–00 | `dice:1d8\|noform\|noparens\|avg` (`1d8`) animal teeth or tusks (no ivory value) |
^item-in-bag
...
> [!gallery]
> ![Map 6.1: Hill Giant Stronghold—Upper Level](file://adventure/TftYP-AtG/001-atg01.webp#gallery)
> ![Player Version](file://adventure/TftYP-AtG/002-atg01-player.webp#gallery)
...
On the upper level of the stronghold, the chance of a random encounter occurs on an hourly basis. At the end of each hour, roll a `dice:d6|noform|noparens|avg` (`d6`). On a roll of 1, which indicates a random encounter, consult the section below that corresponds to the characters' current location.
...
- `dice:1d4+1|noform|noparens|avg` (`1d4 + 1`) young hill giants (use the [orc](3-Mechanics/CLI/bestiary/humanoid/tough-xmm.md) statistics) from the dormitory (area 3) heading through the open yard (area 22) to raid the pantry (area 18A)  

#### Western Section

If the characters are in areas 12 through 21, choose from the following possibilities:

- `dice:2d6|noform|noparens|avg` (`2d6`) orc slaves ([commoners](3-Mechanics/CLI/bestiary/humanoid/commoner-xmm.md)) rushing to get shields (from area 12 or 13) to use as platters  
...
![](file://adventure/TftYP-AtG/002-totyp-06-03.webp#center)
...
> [!gallery]
> ![Map 6.2: Hill Giant Stronghold—Dungeon Level](file://adventure/TftYP-AtG/003-atg02.webp#gallery)
> ![Player Version](file://adventure/TftYP-AtG/005-atg02-player.webp#gallery)
...
If the characters are in areas 1 through 18, area 22, or areas 24 through 33, roll a `dice:d12|noform|noparens|avg` (`d12`). On a roll of 1, which indicates a random encounter, choose from the following possibilities:
...
If the characters are in areas 19 through 21 or area 23, roll a `dice:d20|noform|noparens|avg` (`d20`). On a roll of 1, which indicates a random encounter, choose from the following possibilities:

- `dice:1d4|noform|noparens|avg` (`1d4`) young [giant lizards](3-Mechanics/CLI/bestiary/beast/giant-lizard-xmm.md) from area 21  
- `dice:1d3|noform|noparens|avg` (`1d3`) [carrion crawlers](3-Mechanics/CLI/bestiary/monstrosity/carrion-crawler-xmm.md) coming from area 22 by way of the sinkholes  
...
![](file://adventure/TftYP-AtG/004-totyp-06-04.webp#center)
...
[The Keeper](3-Mechanics/CLI/bestiary/npc/the-keeper-tftyp.md) wears a thick chain mail shirt (accounting for his AC), and he fights with a giant-sized [+1 battleaxe](3-Mechanics/CLI/items/1-weapon-xdmg.md) that gives him a `dice:1d20+10|noform|noparens|text(+10)` bonus to hit. He has a reach of 15 feet with the weapon, which deals `dice:3d8+7|noform|noparens|avg|text(20)` (`3d8 + 7`) slashing damage on a hit.
...
This room holds eight [bugbears](3-Mechanics/CLI/bestiary/fey/bugbear-warrior-xmm.md), alert and ready for trouble. They carry `dice:2d4|noform|noparens|avg` (`2d4`) gp each.
...
Currently, five bugbears—the captain (a [bugbear chief](3-Mechanics/CLI/bestiary/humanoid/bugbear-chief.md)) and four lieutenants ([bugbears](3-Mechanics/CLI/bestiary/fey/bugbear-warrior-xmm.md))—are sitting around a crude table drinking ale and discussing a plan to wipe out the rebel orcs in area 19. All these bugbears have longswords that deal `dice:2d8+2|noform|noparens|avg|text(11)` (`2d8 + 2`) slashing damage on a hit. All five are carrying 50 gp each. The captain also has thirty-one 10 gp gems hidden in a pouch under his armor.
...
![](file://adventure/TftYP-AtG/005-totyp-06-06.webp#center)
...
- Opening the second chest triggers a scything blade trap that targets the opener (`dice:1d20+7|noform|noparens|text(+7)` to hit), dealing `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) slashing damage on a hit. With a successful DC 20 Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) check, a character spots alterations to the hinges that are part of the trap mechanism. Thieves' tools can be used to disable the trap, along with a successful DC 20 Dexterity check. The chest holds bags of coins (totaling 9,100 sp).  
...
![](file://adventure/TftYP-AtG/006-totyp-06-07.webp#center)

Beyond the secret door to this room waits a hidden pit, 30 feet deep, with a trapdoor cover that snaps shut once someone has fallen through it. A character who succeeds on a DC 15 Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) check can discern the edge of the trapdoor. In addition, someone who prods a pit lid forces the pit open with a successful DC 10 Strength check. The lid can be wedged shut with a piton or similar shim. A character doing so must succeed on a DC 15 Strength check to place the shim, or else the shim fails if someone treads on the pit. A creature that falls in takes falling damage plus `dice:2d10|noform|noparens|avg|text(11)` (`2d10`) piercing damage from the spikes. The spikes are also poisoned, so someone injured by them must make a DC 15 Constitution saving throw, taking `dice:4d10|noform|noparens|avg|text(22)` (`4d10`) poison damage on a failed save, or half as much damage on a successful one.
...
> [!gallery]
> ![Map 6.3: Frost Giant Stronghold—Upper Level](file://adventure/TftYP-AtG/007-atg03.webp#gallery)
> ![Player Version](file://adventure/TftYP-AtG/010-atg03-player.webp#gallery)
...
On the upper level of the frost giant stronghold, the chance of a random encounter occurs on an hourly basis. At the end of each hour, roll a `dice:d12|noform|noparens|avg` (`d12`). On a roll of 1, which indicates a random encounter, choose from the following possibilities:

- `dice:1d4+1|noform|noparens|avg` (`1d4 + 1`) [yetis](3-Mechanics/CLI/bestiary/monstrosity/yeti-xmm.md)  
- One [frost giant](3-Mechanics/CLI/bestiary/giant/frost-giant-xmm.md)  
- `dice:1d4+1|noform|noparens|avg` (`1d4 + 1`) [ogres](3-Mechanics/CLI/bestiary/giant/ogre-xmm.md)  
- `dice:1d6+1|noform|noparens|avg` (`1d6 + 1`) [winter wolves](3-Mechanics/CLI/bestiary/monstrosity/winter-wolf-xmm.md)  
...
If loud noise is made in the unoccupied chamber south of the guardroom, the ceiling of ice and icicles in this area partially collapses. A creature beneath the ceiling must make a DC 15 Dexterity saving throw, taking `dice:3d10|noform|noparens|avg|text(16)` (`3d10`) bludgeoning damage on a failed save, or half as much damage on a successful one. Softer noises or visual clues could give warning of the unstable ice.
...
![](file://adventure/TftYP-AtG/008-totyp-06-09.webp#center)
...
Although it certainly appears that some valuables might be scattered among all the bones, in truth there are only a few coins to be had—`dice:1d6|noform|noparens|avg` (`1d6`) each of copper, silver, and gold—and some broken weapons, pottery shards, and the like.
...
This section of the stronghold houses a group of frost giants readying for a raid. Each of the separate areas has piles of hides for sleeping, a table and stools, and a few extra giant-sized weapons around. Each giant here has `dice:2d4|noform|noparens|avg` (`2d4`) rocks to hurl.
...
![](file://adventure/TftYP-AtG/009-totyp-06-11.webp#center)
...
> [!gallery]
> ![Map 6.4: Frost Giant Stronghold—Lower Level](file://adventure/TftYP-AtG/010-atg04.webp#gallery)
> ![Player Version](file://adventure/TftYP-AtG/014-atg04-player.webp#gallery)
...
On the lower level of the frost giant stronghold, the chance of a random encounter occurs on an hourly basis. At the end of each hour, roll a `dice:d10|noform|noparens|avg` (`d10`). On a roll of 1, which indicates a random encounter, choose from the following possibilities:

- One [frost giant](3-Mechanics/CLI/bestiary/giant/frost-giant-xmm.md), a guard making the rounds  
- `dice:1d4+1|noform|noparens|avg` (`1d4 + 1`) [ogres](3-Mechanics/CLI/bestiary/giant/ogre-xmm.md) on an errand for the jarl  
...
Each ogre has `dice:1d100|noform|noparens|avg` (`1d100`) sp, `dice:1d100|noform|noparens|avg` (`1d100`) ep, and `dice:1d100|noform|noparens|avg` (`1d100`) gp in its box or its bag.
...
![](file://adventure/TftYP-AtG/011-totyp-06-12.webp#center)
...
If the magic shield is taken down from the wall, that act releases a spear trap that fires straight ahead to a range of 60 feet. The spear has a `dice:1d20+10|noform|noparens|text(+10)` bonus to hit and deals `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) piercing damage on a hit. The trap mechanism can be seen by someone who examines the shield and succeeds on a DC 15 Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) check. Given 1 minute or so, a character who uses thieves' tools and succeeds on a DC 15 Dexterity check can jam the mechanism so the spear can't launch. If the check fails by 5 or more, the spear launches even if the shield is still on the wall.
...
> [!gallery]
> ![Map 6.5: Hall of the Fire Giant King—Entrance Level](file://adventure/TftYP-AtG/012-atg05.webp#gallery)
> ![Player Version](file://adventure/TftYP-AtG/017-atg05-player.webp#gallery)
...
On the entrance level of the fire giant stronghold, the chance of a random encounter occurs on an hourly basis. At the end of each hour, roll a `dice:d12|noform|noparens|avg` (`d12`). On a roll of 1, which indicates a random encounter, choose from the following possibilities:
...
![](file://adventure/TftYP-AtG/013-totyp-06-14.webp#center)
...
- He has `dice:15d12+90|noform|noparens|avg|text(187)` (`15d12 + 90`) hit points.  
- He has resistance to cold damage.  
- He can speak Common and Giant.  
- His greatsword deals an extra `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) fire damage on a hit.  
...
Three fire giant servants (use the [hill giant](3-Mechanics/CLI/bestiary/giant/hill-giant-xmm.md) statistics, plus immunity to fire damage) are present here, one waiting outside the queen's chamber and two more in the end chamber. Each fights fiercely to protect the queen, with those at the end chamber rushing to aid the other. The servants wield longswords that deal `dice:3d8+5|noform|noparens|avg|text(18)` (`3d8 + 5`) slashing damage on a hit.
...
The chamber to the west of the hydra's lair is filled with stuff, much of which is valueless or nearly so—a pile of 2,800 cp, three mounds of `dice:4d4|noform|noparens|avg` (`4d4`) worthless tapestries and furs, several dozen bales of valueless cloth, scores of various vessels and containers, and urns and vases of pewter and brass and bronze, plus ten urns of silver (worth 50 gp each) and two of gold (worth 500 gp each).
...
**Trunk 4** is locked and trapped. It contains 3 cubic feet of silk, worth 300 gp per cubic foot. If the trunk is opened without the key, a blade springs from the lid and chops down at one target, with a `dice:1d20+11|noform|noparens|text(+11)` bonus to hit and dealing `dice:3d8|noform|noparens|avg|text(13)` (`3d8`) slashing damage on a hit. With a successful DC 20 Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) check, a character can spot alterations to the lock and hinges that are part of the trap mechanism. A successful DC 20 Dexterity check by someone using thieves' tools can be used to disable the hinge mechanism. If the trap goes off but the blade misses, it chops through 2 cubic feet of the silk, ruining it.
...
**Chest 4** is unlocked and trapped. It contains 1,600 gp. If its lid is lifted before the trap is disabled, numerous spikes shoot forward, backward, and to each side. Each spike has a range of 15 feet and a `dice:1d20+11|noform|noparens|text(+11)` bonus to hit, dealing `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) piercing damage on a hit. Anyone within 15 feet of the chest might be hit. With a successful DC 20 Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) check, a character notices that the hinges are connected to four valves, disguised as decorations, that pressurize the spikes in the chest walls. Thieves' tools and a successful DC 20 Dexterity check can be used to disable the valves.
...
![](file://adventure/TftYP-AtG/014-totyp-06-15.webp#center)
...
Within the alcove to the west is a table with a flat chest on it, locked and full of stones (weighing 200 pounds). If the chest is lifted or moved, a pressure plate underneath it shifts, causing six poisoned arrows to shoot out from the north wall. Each arrow has a range of 100 feet and a `dice:1d20+11|noform|noparens|text(+11)` bonus to hit; it deals `dice:1d10|noform|noparens|avg|text(5)` (`1d10`) piercing damage on a hit, and the target struck must make a DC 11 Constitution saving throw, taking `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) poison damage on a failed save, or half as much damage on a successful one). The arrows strike the south wall and shatter if they miss interposing creatures.
...
One warrior has a greatsword, and the other one fights with a human-sized [+2 greataxe](3-Mechanics/CLI/items/2-weapon-xdmg.md). The weapon gives the giant a `dice:1d20+13|noform|noparens|text(+13)` bonus to hit and deals `dice:3d8+9|noform|noparens|avg|text(22)` (`3d8 + 9`) slashing damage. The room also has twenty-one throwing rocks, which all the giants will use if the opportunity presents itself.
...
> [!gallery]
> ![Map 6.6: Hall of the Fire Giant King—Second Level](file://adventure/TftYP-AtG/015-atg06.webp#gallery)
> ![Player Version](file://adventure/TftYP-AtG/021-atg06-player.webp#gallery)
...
On the second level of the fire giant stronghold, the chance of a random encounter occurs on an hourly basis. At the end of each hour, roll a `dice:d12|noform|noparens|avg` (`d12`). On a roll of 1, which indicates a random encounter, choose from the following possibilities:
...
Two [fire giants](3-Mechanics/CLI/bestiary/giant/fire-giant-xmm.md), the King's Torturer and the Royal Headsman, are crouched in the center of the room, playing knucklebones on the floor with stakes of jewelry gems scattered in front of them. The headsman has his gigantic [+2 greataxe](3-Mechanics/CLI/items/2-weapon-xdmg.md) at hand—a weapon that can be employed only by a creature at least as strong and massive as he is. He has a `dice:1d20+13|noform|noparens|text(+13)` bonus to hit with the weapon, and it deals `dice:3d12+9|noform|noparens|avg|text(28)` (`3d12 + 9`) slashing damage on a hit. His friend the torturer has a greatsword nearby.

If the torturer is engaged in melee, he grapples his opponent and attempts to carry or throw the character into the iron maiden and slam it shut, dealing `dice:10d10|noform|noparens|avg|text(55)` (`10d10`) piercing damage to the unlucky victim. A victim can't open the iron maiden from within.
...
If the three [tentacle rods](3-Mechanics/CLI/items/tentacle-rod-xdmg.md) from area 12 are in the characters' possession when the eye appears, and the braziers remain lit, then the altar becomes a means of summoning an Elder Elemental God—it turns a transparent heliotrope in color, the black mass at the center grows larger and shows swollen veins of purple, and the eye is a fiery red-orange. Then a tentacle comes out of the altar and grabs the nearest living creature. The tentacle has a reach of 20 feet and a `dice:1d20+11|noform|noparens|text(+11)` bonus to hit. A creature hit by the tentacle is drawn into the stone—totally gone, destroyed.
...
If a second summoning of this Elder Elemental God is attempted within the same day, roll a `dice:d12|noform|noparens|avg` (`d12`) and apply the indicated result from the table below.

`dice: [](2-against-the-giants.md#^result)`

| dice: d12 | Result |
|-----------|--------|
| 1 | Tentacles try to seize and devour `dice:1d4\|noform\|noparens\|avg` (`1d4`) more creatures within the minute, but entity does not grant any additional desires. |
...
![](file://adventure/TftYP-AtG/016-totyp-06-18.webp#center)
...
![](file://adventure/TftYP-AtG/017-totyp-06-19.webp#center)
...
> [!gallery]
> ![Map 6.7: Hall of the Fire Giant King—Third Level](file://adventure/TftYP-AtG/018-atg07.webp#gallery)
> ![Player Version](file://adventure/TftYP-AtG/025-atg07-player.webp#gallery)
...
On the third level of the fire giant stronghold, the chance of a random encounter occurs on an hourly basis. At the end of each hour, roll a `dice:d12|noform|noparens|avg` (`d12`). On a roll of 1, which indicates a random encounter, choose from the following possibilities:
...
A rope bridge is suspended about 15 feet above the lava river, offering safe access to the northern chamber (area 20). The ropes have been specially treated to keep them from bursting into flame. A creature that comes within 10 feet of the lava for the first time on a turn, or starts its turn within 10 feet of the lava, takes `dice:2d10|noform|noparens|avg|text(11)` (`2d10`) fire damage. A creature that enters or touches the lava for the first time on a turn, or starts its turn in the lava, takes `dice:6d10|noform|noparens|avg|text(33)` (`6d10`) fire damage.
