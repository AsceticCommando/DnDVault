...
A creature takes `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) poison damage every hour it spends in the gas. The gas is light, so it accumulates closer to the ceiling. Inhabitants of the dungeon have immunity to the poisonous effect of the gas.
...
> 
> ![](file://adventure/TftYP-THSoT/000-totyp-03-book.webp#center)
> 
...
Each hour the party is in the ruins, roll a `dice:d12|noform|noparens|avg` (`d12`). On a roll of 1, an encounter occurs. Then roll a `dice:d10|noform|noparens|avg` (`d10`) and see the following table to determine which monsters show up.

`dice: [](2-the-hidden-shrine-of-tamoachan.md#^encounter)`

| dice: d10 | Encounter |
|-----------|-----------|
| 1 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [swarms of rats](3-Mechanics/CLI/bestiary/beast/swarm-of-rats-xmm.md) (diseased, as giant rats) |
| 2 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [swarms of bats](3-Mechanics/CLI/bestiary/beast/swarm-of-bats-xmm.md) |
| 3 | `dice:2d4\|noform\|noparens\|avg` (`2d4`) [giant fire beetles](3-Mechanics/CLI/bestiary/beast/giant-fire-beetle-xmm.md) |
| 4 | `dice:1d6\|noform\|noparens\|avg` (`1d6`) [zombies](3-Mechanics/CLI/bestiary/undead/zombie-xmm.md) |
| 5 | 1 [will-o'-wisp](3-Mechanics/CLI/bestiary/undead/will-o-wisp-xmm.md) |
| 6 | `dice:2d4\|noform\|noparens\|avg` (`2d4`) [baboons](3-Mechanics/CLI/bestiary/beast/baboon-xmm.md) |
| 7 | 1 [swarm of poisonous snakes](3-Mechanics/CLI/bestiary/beast/swarm-of-venomous-snakes-xmm.md) |
| 8 | `dice:1d4\|noform\|noparens\|avg` (`1d4`) [giant frogs](3-Mechanics/CLI/bestiary/beast/giant-frog-xmm.md) |
| 9 | `dice:1d2\|noform\|noparens\|avg` (`1d2`) [panthers](3-Mechanics/CLI/bestiary/beast/panther-xmm.md) |
| 10 | `dice:1d3\|noform\|noparens\|avg` (`1d3`) [giant wolf spiders](3-Mechanics/CLI/bestiary/beast/giant-wolf-spider-xmm.md) |
^encounter
...
> [!gallery]
> ![Map 3.1: The Hidden Shrine of Tamoachan](file://adventure/TftYP-THSoT/001-hs01.webp#gallery)
> ![Player Version](file://adventure/TftYP-THSoT/002-hsotplayer.webp#gallery)
...
![](file://adventure/TftYP-THSoT/002-totyp-03-03.webp#center)
...
![](file://adventure/TftYP-THSoT/003-totyp-03-04.webp#center)
...
![](file://adventure/TftYP-THSoT/004-totyp-03-05.webp#center)
...
Opening the door will release a wave of water. Those hit by the water take `dice:1d4|noform|noparens|avg|text(2)` (`1d4`) bludgeoning damage and must succeed on a DC 15 Strength saving throw or be knocked down and washed along the passage to the north, then westward. A creature that succeeds on a DC 15 Strength ([Athletics](3-Mechanics/CLI/rules/skills.md#Athletics)) check while moving around a corner can grab onto the stonework there and halt its movement. When the flood washes to area 11, the water bursts open the doors there (if they weren't open already), and anyone in the water is dumped into the moat.
...
![](file://adventure/TftYP-THSoT/005-totyp-03-06.webp#center)
...
![](file://adventure/TftYP-THSoT/006-totyp-03-07.webp#center)
...
![](file://adventure/TftYP-THSoT/007-totyp-03-08.webp#center)
...
![](file://adventure/TftYP-THSoT/008-totyp-03-09.webp#center)
...
After a victim has been trapped for 5 rounds, the cover to a 5-foot-square pit starts to drop open. The pit is lined with spikes. The cover fully opens in 2 rounds, at which point the arms swiftly spring back into the lintel, releasing any victims to plummet into the 15-foot-deep pit. A fall into the pit deals `dice:2d10|noform|noparens|avg|text(11)` (`2d10`) bludgeoning damage. The spikes are hard rubber wrapped around balsa wood, placed for artistic effect.
...
The door is trapped, but the only evidence of this fact is concealed within the door frame and under the stucco on the ceiling, completely hidden. When the doors are pushed open, five heavy crossbows, set in the ceiling of the room beyond, fire. Two are aimed toward each door, and the last is aimed down the center of the two doors. The crossbows have a `dice:1d20+6|noform|noparens|text(+6)` bonus to hit, and each deals `dice:1d10|noform|noparens|avg|text(5)` (`1d10`) piercing damage.
...
The first time a creature or an object enters or touches the silvery liquid metal of the sea or the rivers on a turn, or a creature starts its turn in such a place, the creature or object takes `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) fire damage. If any of this fluid is carried out of this room, it evaporates in 1 minute. The ships and the coffin floating on the liquid deal similar fire damage if they are touched.
...
When the catch is pressed, a lever wedged in the lid pops up, triggering the launch of darts. The tubes that hold these darts are sealed with a thin layer of stucco, so they're all but impossible to discover without damaging the chest. Someone who cautiously opens the chest can feel the pressure from the lever and see it by peeking into the chest. If someone can reach the lever through the half-open lid and hold it in place, the trap doesn't function. Holding the lever in place as the lid is opened requires a successful DC 12 Strength check. This check must be repeated in every round that the lever is held down. If the trap triggers, four darts fire from each of the chest's front and back sides. Each dart flies up to 60 feet, has a `dice:1d20+6|noform|noparens|text(+6)` bonus to hit, and deals `dice:2d4|noform|noparens|avg|text(5)` (`2d4`) piercing damage on a hit.
...
If the pressure plate is depressed, two of the carved warriors pivot out from the walls in front of the party, crossing their metal halberds before them to bar the way to the northern doors. The blades spark and hum when they are brought together. A creature that touches either of the blades takes `dice:2d4|noform|noparens|avg|text(5)` (`2d4`) lightning damage and must succeed on a DC 15 Constitution saving throw or become [paralyzed](3-Mechanics/CLI/rules/conditions.md#Paralyzed) while it is in contact. A [paralyzed](3-Mechanics/CLI/rules/conditions.md#Paralyzed) creature takes the damage again at the start of each of its turns.
...
![](file://adventure/TftYP-THSoT/009-totyp-03-10.webp#center)
...
The third and sixth bars along the way are corroded. When a character moves onto one of these bars, roll a `dice:d20|noform|noparens|avg` (`d20`). On a roll of 5 or lower, or 2 or lower for a Small character, the bar breaks. If the bar doesn't break and the roll is 10 or lower, some obvious sign of the bar's weakness becomes evident, such as chipping or bending.
...
Someone who falls into the pit always comes down on a bush, cushioning the impact. The fall deals only `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) bludgeoning damage, but the character lands on a plant's adhesive blossoms.
...
The first creature to come within 10 feet of the mirror at the end of the corridor must make a DC 15 Wisdom saving throw. On a failed check, the creature believes that it is in deadly combat with a feathered warrior that has emerged from the mirror. This effect is like that of a [phantasmal force](3-Mechanics/CLI/spells/phantasmal-force-xphb.md) spell, with the character battling a warrior that has a `dice:1d20+6|noform|noparens|text(+6)` bonus to hit and deals `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) psychic damage, as described in the spell. The effect ends if the affected creature deals the imaginary warrior a total amount of damage equal to the creature's own hit point maximum.
...
- Someone who tries climbing up the shelves takes `dice:1d4|noform|noparens|avg|text(2)` (`1d4`) force damage and is pushed back 10 feet.  
...
- The eastern stairs are shrouded in dim light, which can't be made brighter by any means. Any character who mounts the stairs falls down and rolls back onto the floor, taking `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) bludgeoning damage.  
...
A creature that touches the red mirror for the first time on a turn takes `dice:1d4|noform|noparens|avg|text(2)` (`1d4`) fire damage.
...
![](file://adventure/TftYP-THSoT/011-totyp-03-12.webp#center)
...
![](file://adventure/TftYP-THSoT/012-totyp-03-14.webp#center)
...
![](file://adventure/TftYP-THSoT/013-totyp-03-15.webp#center)
