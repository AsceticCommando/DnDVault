...
![](file://adventure/OotA/070-ooa09-01.webp#center)
...
> [!gallery]
> ![Map 9.1: Mantol-Derith](file://adventure/OotA/071-09-01.webp#gallery)
> ![Player Version](file://adventure/OotA/072-mantol-derrith-player-version.webp#gallery)

## 1. Secret Entrances

Mantol-Derith is hidden behind 10-foot-square secret doors carved to resemble walls of natural stone. A creature within 5 feet of a secret door can make a DC 15 Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) check to spot it. Each secret door has an [arcane lock](3-Mechanics/CLI/spells/arcane-lock-xphb.md) spell cast on it. Speaking the proper password within 5 feet of a secret door opens it. Opening a secret door by any means other than the correct password triggers a magical disintegration trap on whichever side the door is opened from. Each creature in the 20-foot-square area directly in front of the secret door must succeed on a DC 15 Dexterity saving throw, taking `dice:10d10|noform|noparens|avg|text(55)` (`10d10`) force damage on a failed save, or half as much damage on a successful one. A creature reduced to 0 hit points by this trap is disintegrated, leaving only its possessions behind.
...
![](file://adventure/OotA/073-ooa09-03.webp#center)
...
![](file://adventure/OotA/074-ooa09-04.webp#center)
...
The svirfneblin and duergar markets are clustered on the east side of the trench. Most of their tents and stalls have been knocked over, and various goods (see "Treasure") are strewn upon the cavern floor amid the slaughtered corpses of giant lizards and steeders. Characters approaching the market see two enlarged [duergar](3-Mechanics/CLI/bestiary/humanoid/spy-xmm.md) chasing a pair of [deep gnomes](3-Mechanics/CLI/bestiary/humanoid/scout-xmm.md) who have cast [blur](3-Mechanics/CLI/spells/blur-xphb.md) spells on themselves. A third unarmed [deep gnome](3-Mechanics/CLI/bestiary/humanoid/scout-xmm.md) is hiding in plain sight under the wreckage of a zurkhwood stall. He has a pale leather bag tucked under one arm.
...
![](file://adventure/OotA/075-ooa09-06.webp#center)
...
The large pool east of the market is fed by small waterfalls pouring through cracks in the cavern ceiling. The water is fresh and safe to drink. Growing along the shore are `dice:3d6|noform|noparens|avg` (`3d6`) waterorbs (see "Fungi of the Underdark").

#### Treasure

Characters who search the marketplace find a total of `dice:2d6|noform|noparens|avg` (`2d6`) 50 gp gems scattered about.
...
![](file://adventure/OotA/076-ooa09-05.webp#center)
...
The drow sleep in cots arranged in neat rows, while the merchants store their valuable wares in three stone buildings. The door to each building is protected by a [glyph of warding](3-Mechanics/CLI/spells/glyph-of-warding-xphb.md) (save DC 14) that triggers an explosive runes effect (`dice:5d8|noform|noparens|avg` (`5d8`) thunder damage) when a creature other than a drow opens it.

#### Treasure

Each storage building contains `dice:2d6|noform|noparens|avg` (`2d6`) crystal vials of exotic perfume worth 100 gp each, `dice:3d6|noform|noparens|avg` (`3d6`) crystal bottles of distilled beverages worth 25 gp each, `dice:1d4|noform|noparens|avg` (`1d4`) magic items (determined by rolling on Magic Item Table A in "chapter 7" of the *Dungeon Master's Guide*), and `dice:2d6|noform|noparens|avg` (`2d6`) vials of carrion crawler mucus (see ""Poisons"" in chapter 8 of the *Dungeon Master's Guide*). Characters also find `1d10 × 10` pounds of food and `1d10 × 5` gallons of water stored in each building.
...
Guarding the fungi grove are four [invisible](3-Mechanics/CLI/rules/conditions.md#Invisible) [duergar](3-Mechanics/CLI/bestiary/humanoid/spy-xmm.md) that attack intruders on sight. Four more [invisible](3-Mechanics/CLI/rules/conditions.md#Invisible) [duergar](3-Mechanics/CLI/bestiary/humanoid/spy-xmm.md) stand in front of the double doors and attack any non-duergar that come into view.

![](file://adventure/OotA/077-ooa09-07.webp#center)
...
- `dice:2d6|noform|noparens|avg` (`2d6`) battleaxes  
- `dice:2d6|noform|noparens|avg` (`2d6`) glaives  
- `dice:2d6|noform|noparens|avg` (`2d6`) lances  
- `dice:2d6|noform|noparens|avg` (`2d6`) longswords  
- `dice:2d6|noform|noparens|avg` (`2d6`) morningstars  
- `dice:2d6|noform|noparens|avg` (`2d6`) war picks  
- `dice:2d6|noform|noparens|avg` (`2d6`) warhammers  
- `dice:1d6|noform|noparens|avg` (`1d6`) suits of half-plate  
- `dice:1d6|noform|noparens|avg` (`1d6`) suits of chainmail  
- `dice:1d6|noform|noparens|avg` (`1d6`) suits of splint mail  
- `dice:1d6|noform|noparens|avg` (`1d6`) suits of plate mail  

An iron safe stands in a corner of the storage building. The safe weighs 1,000 pounds, is locked, and contains `dice:1d4|noform|noparens|avg` (`1d4`) magic items (determined by rolling on Magic Item Table B in "chapter 7" of the *Dungeon Master's Guide*). Ghuldur knows the combination to unlock the safe, which otherwise requires a [knock](3-Mechanics/CLI/spells/knock-xphb.md) spell or similar magic to open.
...
Set into the northeast wall is a set of stone double doors that the svirfneblin have barred from within. Four enlarged [duergar](3-Mechanics/CLI/bestiary/humanoid/spy-xmm.md) are breaking through the doors with war picks, and it's only a matter of time before they reduce the doors to rubble. Eight more [invisible](3-Mechanics/CLI/rules/conditions.md#Invisible) [duergar](3-Mechanics/CLI/bestiary/humanoid/spy-xmm.md) stand nearby, ready to storm the svirfneblin warehouse as soon as the doors crumble.

#### Treasure

Each dead svirfneblin carries a pouch that contains `dice:1d4|noform|noparens|avg` (`1d4`) 50 gp gems.
...
Half way up the southeast wall is a hidden compartment. Finding it requires a successful DC 19 Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) check. The compartment contains `dice:1d4|noform|noparens|avg` (`1d4`) magic items (determined by rolling on Magic Item Table C in "chapter 7" of the *Dungeon Master's Guide*).
...
Perched on a ledge overlooking the pavilion is a [gargoyle](3-Mechanics/CLI/bestiary/elemental/gargoyle-xmm.md) touched by the madness of Fraz-Urb'luu's gem. Characters with a passive Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) score of 13 or higher spot the creature. It's waiting for [Kinyel Druu'giir](3-Mechanics/CLI/bestiary/npc/kinyel-druugiir-oota.md), its "true love," to emerge from the pavilion.
...
![](file://adventure/OotA/078-ooa09-08.webp#center)
...
![](file://adventure/OotA/079-ooa09-09.webp#center)
...
> - **Shed 2** holds `dice:1d10|noform|noparens|avg` (`1d10`) backpacks, `dice:2d10|noform|noparens|avg` (`2d10`) bedrolls, `dice:2d10|noform|noparens|avg` (`2d10`) blankets, `dice:2d6|noform|noparens|avg` (`2d6`) climber's kits, and `dice:2d6|noform|noparens|avg` (`2d6`) healer's kits.  
> - **Shed 3** contains `dice:3d6|noform|noparens|avg` (`3d6`) hooded lanterns, `dice:3d6|noform|noparens|avg` (`3d6`) flasks of oil, `dice:3d6|noform|noparens|avg` (`3d6`) 50-foot coils of hempen rope, `dice:3d6|noform|noparens|avg` (`3d6`) grappling hooks, and `dice:3d6|noform|noparens|avg` (`3d6`) tinderboxes.  
> - **Shed 4** holds five crates, each one containing `dice:1d20|noform|noparens|avg` (`1d20`) days of rations.  
> - **Shed 5** contains `dice:2d10|noform|noparens|avg` (`2d10`) empty glass bottles, `dice:2d6|noform|noparens|avg` (`2d6`) scroll cases, `dice:2d10|noform|noparens|avg` (`2d10`) copper tankards, `dice:2d10|noform|noparens|avg` (`2d10`) iron pots, `dice:2d10|noform|noparens|avg` (`2d10`) empty sacks, and `dice:2d10|noform|noparens|avg` (`2d10`) empty waterskins.  
> - **Shed 6** holds `dice:2d10|noform|noparens|avg` (`2d10`) empty wooden chests with iron fittings and `dice:2d10|noform|noparens|avg` (`2d10`) locks with keys.  
> - **Shed 7** contains `dice:2d6|noform|noparens|avg` (`2d6`) jars of ink, `dice:3d12|noform|noparens|avg` (`3d12`) blank pieces of parchment, `dice:3d6|noform|noparens|avg` (`3d6`) boxes of candles (12 candles per box), and `dice:2d6|noform|noparens|avg` (`2d6`) ink pens.  
> - **Shed 8** holds `dice:3d6|noform|noparens|avg` (`3d6`) crowbars, `dice:3d6|noform|noparens|avg` (`3d6`) hammers, and `dice:3d6|noform|noparens|avg` (`3d6`) miner's picks.  
...
> - **Shed 10** contains `dice:3d6|noform|noparens|avg` (`3d6`) jars of spices worth 10 gp each, `dice:2d6|noform|noparens|avg` (`2d6`) 10-pound sacks of cinnamon worth 20 gp each, and `dice:3d6|noform|noparens|avg` (`3d6`) boxes of soap (12 bars of soap per box).  
> - **Shed 11** holds four wooden chicken coops, each one containing `dice:1d6+1|noform|noparens|avg` (`1d6 + 1`) live chickens. Sacks of chicken feed are stored on a shelf above the coops.  
...
![](file://adventure/OotA/080-ooa09-11.webp#center)
...
On a 30-foot-high shelf overlooking the cave is an old grick nest. Hidden here are three [potions of vitality](3-Mechanics/CLI/items/potion-of-vitality-xdmg.md). Climbing the wall to reach the shelf requires a successful DC 10 Strength ([Athletics](3-Mechanics/CLI/rules/skills.md#Athletics)) check.
...
![](file://adventure/OotA/081-ooa09-10.webp#center)
...
- She wields a warhammer instead of a longsword (`dice:1d8+3|noform|noparens|avg` (`1d8 + 3`) bludgeoning damage on a hit, or `dice:1d10+3|noform|noparens|avg` (`1d10 + 3`) bludgeoning damage if using the weapon with two hands).  
...
![](file://adventure/OotA/082-ooa09-12.webp#center)

If [Peebles](3-Mechanics/CLI/bestiary/npc/peebles-oota.md) or the characters harvest [Lorthuun](3-Mechanics/CLI/bestiary/npc/lorthuun-oota.md)'s eyestalks (see the "Beholder Eyestalks" sidebar), [Peebles](3-Mechanics/CLI/bestiary/npc/peebles-oota.md) asks to join the party as it leaves Mantol-Derith. After traveling with them for a tenday, [Peebles](3-Mechanics/CLI/bestiary/npc/peebles-oota.md) urges the characters to deviate from their preferred route, claiming that they're dangerously close to a nest of purple worms. [Peebles](3-Mechanics/CLI/bestiary/npc/peebles-oota.md) uses this lie to propose an alternate route. If the characters follow this detour, [Peebles](3-Mechanics/CLI/bestiary/humanoid/scout-xmm.md) leads them to the cavern of [Xazax the Eyemonger](3-Mechanics/CLI/bestiary/npc/xazax-the-eyemonger-oota.md), a terrifying [beholder](3-Mechanics/CLI/bestiary/aberration/beholder-xmm.md) that kills others of its kind and grafts their eyestalks to its body. [Peebles](3-Mechanics/CLI/bestiary/npc/peebles-oota.md) heads to Xazax's cave regardless of whether the characters follow him or not.
