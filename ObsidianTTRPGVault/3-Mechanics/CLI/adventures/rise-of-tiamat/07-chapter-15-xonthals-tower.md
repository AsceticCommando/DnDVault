...
![](file://adventure/RoT/022-tod-15-01.webp#center)
...
> [!gallery]
> ![Map 15.1: Xonthal's Tower](file://adventure/RoT/023-map-15-1-xonthals-tower.webp#gallery)
> ![Player Version](file://adventure/RoT/024-map-15-1-xonthals-tower-player.webp#gallery)
...
- Dumping the kettle into the fire reduces damage to `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) poison damage on a failed save, or half damage on a successful save. Smoke from burning drakeswort is less dangerous than steam.  
...
Each time a suit of armor is destroyed, roll a `dice:d6|noform|noparens|avg` (`d6`). On a roll of 6, a large bloodstone rolls out of the armor as its pieces fall to the ground. If the bloodstone is carried out of this area, either path leads the characters back to the sundial.
...
The skeletal forelimbs of a black dragon have been mounted to the altar and enchanted with a permanent [animate objects](3-Mechanics/CLI/spells/animate-objects-xphb.md) spell. When the characters enter, it won't be apparent that the limbs are connected to the altar or that they can be animated. Each limb has AC 10 and 50 hit points and can make one melee attack per round at a creature within 5 feet of it: `dice:1d20+4|noform|noparens|text(+4)` to hit; `dice:1d10+1|noform|noparens|avg|text(6)` (`1d10 + 1`) piercing damage.
...
![Iskander](file://adventure/RoT/025-tod-15-02.webp#center)
