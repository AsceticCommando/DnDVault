...
![](file://adventure/RoT/007-tod-10-01.webp#center)
...
![Maccath the Crimson](file://adventure/RoT/008-tod-10-02.webp#center)
...
Searching the Sea of Moving Ice for the plateau-like iceberg described by Maccath is a time-consuming process. Roll a `dice:d6|noform|noparens|avg` (`d6`) each morning, afternoon, and night. On a roll of 1, an encounter occurs. Roll on the table to determine the specifics, adding +1 for each previous table roll made while the characters were searching from the ship on water by day. Searching by night or searching by day while *Frostskimmr* is on an ice floe doesn't improve the odds of finding Oyaviggaton.
...
Any character who eats the poisoned fish must make a DC 15 Constitution saving throw. Failure means the character takes `dice:3d8|noform|noparens|avg|text(13)` (`3d8`) poison damage and becomes [unconscious](3-Mechanics/CLI/rules/conditions.md#Unconscious) for 8 hours. On a successful save, the character takes `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) poison damage. If all the characters are rendered [unconscious](3-Mechanics/CLI/rules/conditions.md#Unconscious) by the poison, they are tied up and carried down to area 1 in the ice caves, where they awaken to see 5 [ice toads](3-Mechanics/CLI/bestiary/monstrosity/ice-toad-rot.md) (see appendix D) examining their belongings.
...
|-----------|--------------------|
| 1–2 | Fog |
| 3–5 | [Kobolds](3-Mechanics/CLI/bestiary/dragon/kobold-warrior-xmm.md) (`dice:3d6\|noform\|noparens\|avg` (`3d6`)) |
| 6–8 | [Ice toads](3-Mechanics/CLI/bestiary/monstrosity/ice-toad-rot.md) (`dice:1d6\|noform\|noparens\|avg` (`1d6`)) |
| 9 | [Maccath the Crimson](3-Mechanics/CLI/bestiary/npc/maccath-the-crimson-rot.md) |
| 10 | [Ice trolls](3-Mechanics/CLI/bestiary/giant/ice-troll-rot.md) (`dice:1d2\|noform\|noparens\|avg` (`1d2`)) |
^encounter-or-event
...
The chute drops 60 feet to the top of the ice platform in area 16 in Arauthator's lair. A creature tipped into the chute slides as much as falls, taking only `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) bludgeoning damage when hitting bottom. The creature must then attempt a Dexterity saving throw or fall off the ice platform; see area 16 for more information. A creature that jumps into the chute on purpose takes half damage from the descent and has advantage on the saving throw to stay on the ice platform.
