---
obsidianUIMode: preview
cssclasses:
- json5e-monster
tags:
- ttrpg-cli/compendium/src/5e/oow
- ttrpg-cli/monster/cr/1-4
- ttrpg-cli/monster/size/medium
- ttrpg-cli/monster/type/humanoid/any-race
statblock: inline
statblock-link: "#^statblock"
aliases:
- "Gildha Duhn"
---
# [Gildha Duhn](3-Mechanics/CLI/bestiary/npc/gildha-duhn-oow.md)
*Source: The Orrery of the Wanderer p. 110*  

```statblock
"name": "Gildha Duhn (OoW)"
"size": "Medium"
"type": "humanoid"
"subtype": "any race"
"alignment": "Any alignment"
"ac": !!int "10"
"hp": !!int "9"
"hit_dice": "2d8"
"modifier": !!int "0"
"stats":
  - !!int "10"
  - !!int "10"
  - !!int "10"
  - !!int "10"
  - !!int "14"
  - !!int "11"
"speed": "30 ft."
"skillsaves":
  - "name": "[Medicine](3-Mechanics/CLI/rules/skills.md#Medicine)"
    "desc": "+4"
  - "name": "[Religion](3-Mechanics/CLI/rules/skills.md#Religion)"
    "desc": "+2"
"gear":
  - "[club](3-Mechanics/CLI/items/club-xphb.md)"
"senses": "[darkvision](3-Mechanics/CLI/rules/senses.md#Darkvision) 60 ft., passive\
  \ Perception 12"
"languages": "any one language (usually Common), Orc"
"cr": "1/4"
"traits":
  - "desc": "Gildha is a 1st-level spellcaster. Its spellcasting ability is Wisdom\
      \ (spell save DC 12, +4 to hit with spell attacks). Gildha has following cleric\
      \ spells prepared:\n\n**Cantrips (at will):** [light](3-Mechanics/CLI/spells/light-xphb.md),\
      \ [sacred flame](3-Mechanics/CLI/spells/sacred-flame-xphb.md), [thaumaturgy](3-Mechanics/CLI/spells/thaumaturgy-xphb.md)\n\
      \n**1st level (3 slots):** [bless](3-Mechanics/CLI/spells/bless-xphb.md), [cure\
      \ wounds](3-Mechanics/CLI/spells/cure-wounds-xphb.md), [sanctuary](3-Mechanics/CLI/spells/sanctuary-xphb.md)"
    "name": "Spellcasting"
  - "desc": "Gildha"
    "name": "Relentless Endurance"
"actions":
  - "desc": "*Melee Weapon Attack:* +2 to hit, reach 5 ft., one target. *Hit:* 2 (1d4)\
      \ bludgeoning damage."
    "name": "Club"
"source":
  - "OoW"
"image": "file://bestiary/tokens/OoW/Gildha%20Duhn.webp"
```
^statblock